...
async function runExcelLayoutRegression() {
  ...
  assert.strictEqual(worksheet.getRow(1).getCell(1).value, 'MOREN MALİ MÜŞAVİRLİK', 'Excel eski Moren başlığına dönmeli');
  assert.strictEqual(worksheet.getRow(2).getCell(1).value, 'KDV Kontrol Raporu', 'Excel eski rapor alt başlığını korumalı');
  assert.strictEqual(worksheet.getRow(4).getCell(1).value, 'Mükellef', 'Excel eski bilgi bloğu korunmalı');
  assert.strictEqual(worksheet.getRow(4).getCell(3).value, 'SERVİS PERSONEL TAŞIMACILIĞI TURİZM LİMİTED ŞİRKETİ', 'Excel üst bilgide mükellef adı korunmalı');
  assert.strictEqual(worksheet.getRow(4).getCell(5).value, 'Dönem', 'Excel üst bilgide dönem etiketi korunmalı');
  assert.strictEqual(worksheet.getRow(4).getCell(6).value, '2026/04', 'Excel üst bilgide dönem korunmalı');
  assert.strictEqual(worksheet.getRow(8).getCell(1).value, 'ÖZET', 'Excel eski özet bloğu korunmalı');
  assert.strictEqual(headerRow.getCell(1).value, '#', 'Excel ana tablo başlığı eski 15. satırda başlamalı');
  assert.strictEqual(row.getCell(1).value, 1, 'Excel veri satırı eski 16. satırda başlamalı');
  assert.strictEqual(row.getCell(12).alignment.wrapText, true, 'Eski Excel düzeninde öneri hücresi sarma davranışını korumalı');
  assert.strictEqual(row.getCell(13).alignment.wrapText, true, 'Eski Excel düzeninde içerik yorumu sarma davranışını korumalı');

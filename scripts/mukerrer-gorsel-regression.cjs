...
(async () => {
  ...
  console.log('4) revalidate — MUKERRER_FIS / MUKERRER_GORSEL / sahip kararı');
  {
    ...
    // görsel benzerlik
    const gor = makeService({ docs: [fis({ id: 'd0', belgeNo: '0001', createdAt: new Date('2026-07-30T10:00:00Z'), imagePhash: '3e2f2f272f37332f' }), fis({ id: 'd1', belgeNo: '0002', imagePhash: '0e2f2f272f27336f' })], taxpayer: TP });
    const vG = await gor.svc.revalidateDocument('t', 'd1');
    const uG = uyOf(gor.state).find((u) => u.kod === 'MUKERRER_GORSEL');
    assert(vG.status === 'OK' && uG && uG.seviye === 'uyari' && uG.meta.hamming === 4 && uG.meta.ilkBelgeId === 'd0' && /ikinci fotoğrafı/.test(uG.baslik), 'imagePhash Hamming 4 → MUKERRER_GORSEL uyarı (meta.hamming=4, ilkBelgeId)');
    ...
    const mk = makeService({ docs: [fis({ id: 'd0', belgeNo: '0001', createdAt: new Date('2026-07-30T10:00:00Z'), imagePhash: '3e2f2f272f37332f' }), fis({ id: 'd1', belgeNo: '0002', imagePhash: '3e2f2f272f37332f' })], taxpayer: TP });

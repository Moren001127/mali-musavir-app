...
function BelgeCipi({ harf, label, belge, goruldu, onClick }: { harf: string; label: string; belge: BelgeParcasi; goruldu: boolean; onClick: () => void }) {
  ...
  if (!belge || !belge.pdfVar) {
    return (
      <span data-pa-belge="yok" className={taban} title={!belge ? `${label} bu dönemde yok` : `${label} PDF'i henüz inmedi`} aria-label={`${label} yok`}
  ...
  return (
    <button type="button" onClick={onClick}
      ...
      aria-label={`${label} aç`}
      data-pa-belge={goruldu ? 'goruldu' : 'yeni'}
...
function MahiyetRozeti({ mahiyet }: { mahiyet?: string | null }) {
  ...
  if (!m) return <span data-pa-faint style={portalStyle({ color: 'rgba(250,250,249,0.3)' })}>—</span>;
  if (m === 'ASIL') return <span data-pa-mahiyet="asil" className="text-[12px]" style={portalStyle({ color: 'rgba(250,250,249,0.55)' })}>ASIL</span>;
  const kirmizi = /IPTAL|İPTAL/.test(m);
  return (
    <span data-pa-mahiyet={kirmizi ? 'iptal' : 'ek'} className="inline-flex items-center rounded-full px-2 py-[2px] text-[10.5px] font-bold tracking-wide"
...
function DurumSeridi({ donemKaydi, sifreliMukellef, aktifIs, hataSayisi, sifreBekleyen, runnerAcik, onHata }: {
  ...
  const madde = (ikon: React.ReactNode, deger: React.ReactNode, etiket: string) => (
    <span data-pa-madde className="inline-flex items-center gap-2 text-[12.5px]" style={portalStyle({ color: 'rgba(250,250,249,0.6)' })}>
      <span data-pa-madde-ikon className="flex h-6 w-6 items-center justify-center rounded-full" style={portalStyle({ background: 'rgba(212,184,118,0.12)', color: GOLD })}>{ikon}</span>
      <b className="tabular-nums" style={portalStyle({ color: METIN })}>{deger}</b> {etiket}
    </span>
  );
  const sorun = hataSayisi > 0 || sifreBekleyen > 0;
  return (
    <div data-pa-durum-seridi className="flex flex-wrap items-center gap-x-6 gap-y-2 px-4 py-3" style={portalStyle({ borderBottom: '1px solid rgba(255,255,255,0.07)' })}>
      ...
      <span className="ml-auto flex items-center gap-3">
        ...
        {sorun ? (
          ...
          <span data-pa-hap="yesil" className="inline-flex h-7 items-center gap-1.5 rounded-full px-3 text-[11.5px] font-semibold" style={portalStyle({ background: 'rgba(92,191,138,0.10)', border: '1px solid rgba(92,191,138,0.3)', color: '#5cbf8a' })}>
...
function SgkBildirgeModuleIc() {
  ...
  return (
    <div data-pa-module="sgk" className="space-y-4">
      <div data-pa-table className="overflow-hidden rounded-2xl border" style={portalStyle({ borderColor: 'rgba(255,255,255,0.07)', background: 'linear-gradient(160deg, rgba(255,255,255,0.035), rgba(255,255,255,0.012))', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.04), 0 12px 30px rgba(0,0,0,0.25)' })}>
        <DurumSeridi
          donemKaydi={toplam}
        ...
        <div data-pa-toolbar="satir" className="flex flex-wrap items-center gap-2 px-4 py-3" style={portalStyle({ borderBottom: KENAR })}>
          ...
          <div className="relative">
            <select value={turFilter} onChange={(e) => setTurFilter(e.target.value)} aria-label="SGK türü" data-pa-field className="pl-9 pr-8 text-[12.5px] outline-none border appearance-none min-w-[136px]" style={portalStyle({ ...kutuStili, width: 'auto' })}>
          ...
          <div className="relative" ref={donemRef}>
            <button type="button" onClick={() => setDonemOpen((o) => !o)} aria-label="Dönem"
              ...
              <span data-pa-donem-metin={queryMonth ? 'secili' : 'bos'} className="truncate" style={portalStyle({ color: queryMonth ? METIN : 'rgba(250,250,249,0.55)' })}>
            ...
            {donemOpen && (
              <div data-pa-dropdown className="absolute z-50 mt-1.5 left-0 w-[270px] rounded-xl border p-3" style={portalStyle({ background: '#1a1410', borderColor: 'rgba(212,184,118,0.25)', boxShadow: '0 16px 40px rgba(0,0,0,0.5)' })}>
                <div className="flex items-center justify-between mb-2.5">
                  <button type="button" onClick={() => setQueryYear(String(Number(queryYear) - 1))} data-pa-btn="ikincil" className="h-7 w-7 grid place-items-center rounded-lg border hover:brightness-125" style={portalStyle({ borderColor: 'rgba(255,255,255,0.1)', color: METIN })} aria-label="Önceki yıl"><ChevronLeft size={15} /></button>
                  <span className="text-[14px] font-bold tabular-nums" style={portalStyle({ color: METIN })}>{queryYear}</span>
                  <button type="button" onClick={() => setQueryYear(String(Number(queryYear) + 1))} data-pa-btn="ikincil" className="h-7 w-7 grid place-items-center rounded-lg border hover:brightness-125" style={portalStyle({ borderColor: 'rgba(255,255,255,0.1)', color: METIN })} aria-label="Sonraki yıl"><ChevronRight size={15} /></button>
                </div>
                ...
                <div className="flex items-center justify-between mt-2.5 pt-2.5" style={portalStyle({ borderTop: '1px solid rgba(255,255,255,0.08)' })}>
                  <button type="button" onClick={() => { setQueryMonth(''); setDonemOpen(false); }} data-pa-link="notr" className="text-[12px] font-semibold hover:brightness-125" style={portalStyle({ color: 'rgba(250,250,249,0.6)' })}>Tüm dönemler</button>
                  <button type="button" onClick={() => { setQueryMonth(String(now.getMonth() + 1).padStart(2, '0')); setQueryYear(String(now.getFullYear())); setDonemOpen(false); }} data-pa-link="civit" className="text-[12px] font-semibold hover:brightness-125" style={portalStyle({ color: GOLD })}>Bu ay</button>
                </div>
              </div>
            )}
          </div>
            ...
            <tbody style={portalStyle({ color: 'rgba(250,250,249,0.88)', opacity: docsQuery.isPlaceholderData ? 0.55 : 1, transition: 'opacity .15s' })}>
              ...
              {!docsQuery.isLoading && !docsQuery.isError && rows.length === 0 && (
                <tr><td colSpan={8} className="px-3 py-14 text-center" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>
                  <span data-pa-bos-ikon className="mx-auto mb-2 flex h-11 w-11 items-center justify-center rounded-full" style={portalStyle({ background: 'rgba(212,184,118,0.12)', color: GOLD })}><ShieldCheck size={18} /></span>
                    ...
                    return (
                      <tr key={d.id} className="transition-colors hover:bg-white/[0.03]" style={portalStyle({ background: i % 2 ? 'rgba(255,255,255,0.012)' : 'transparent' })}>
                        <td className="px-3 py-2.5 align-middle" style={portalStyle({ borderBottom: KENAR })}>
                          <div data-pa-ad className="truncate text-[13px] font-semibold" style={portalStyle({ color: METIN })} title={ad}>{ad}</div>
                          {d.taxpayer?.taxNumber && <div data-pa-vkn className="text-[11px] tabular-nums" style={portalStyle({ color: 'rgba(250,250,249,0.38)' })}>{d.taxpayer.taxNumber}</div>}
                        </td>
                        ...
                        <td className="px-3 py-2.5 align-middle text-right tabular-nums whitespace-nowrap" style={portalStyle({ borderBottom: KENAR })}>
                          {tutar !== null && Number.isFinite(tutar)
                            ...
                            : <span data-pa-faint style={portalStyle({ color: 'rgba(250,250,249,0.3)' })}>—</span>}
                        </td>
                        <td className="px-3 py-2.5 align-middle" style={portalStyle({ borderBottom: KENAR })}>

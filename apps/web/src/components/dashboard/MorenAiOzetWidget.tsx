...
export function MorenAiOzetWidget() {
  ...
  return (
    <section
      data-dashboard-surface data-dashboard-root className="rounded-2xl p-5 relative overflow-hidden"
      style={portalStyle({
    ...
    >
      ...
      <div data-dashboard-band="plum" className="flex items-center justify-between mb-4 gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Brain size={14} style={portalStyle({ color: GOLD })} />
            <span className="text-[10.5px] uppercase font-bold tracking-[.18em]" style={portalStyle({ color: GOLD })}>
              MOREN AI
            </span>
          </div>
          <p className="text-[12px] mt-1" style={portalStyle({ color: 'rgba(250,250,249,0.52)' })}>
        ...
        {metrics.map((metric) => {
          ...
          return (
            <Link
              ...
              style={portalStyle({ background: 'rgba(255,255,255,0.025)', border: `1px solid ${metric.color}22` })}
            >
              <div className="flex items-center gap-1.5 mb-1.5">
                <Icon size={12} style={portalStyle({ color: metric.color })} />
                <span className="text-[9.5px] uppercase font-bold tracking-[.14em]" style={portalStyle({ color: 'rgba(250,250,249,0.55)' })}>
                  {metric.label}
                </span>
              </div>
              <p
              ...
              <p className="text-[10px] mt-1" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>
                {metric.sub}
              </p>
            </Link>
          );
      ...
      {bugunHata > 0 && (
        <Link
          ...
          style={portalStyle({ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.22)' })}
        >
          <AlertTriangle size={13} style={portalStyle({ color: '#fca5a5' })} />
          <span className="text-[12px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>

  ...
  return (
    <svg width="68" height="68" viewBox="0 0 68 68" className="shrink-0">
      ...
      <circle cx="34" cy="34" r={r} fill="none" stroke="url(#bugun-halka)" strokeWidth="5" strokeLinecap="round"
        strokeDasharray={`${c * dolu} ${c}`} transform="rotate(-90 34 34)" style={{ filter: `drop-shadow(0 0 6px ${rgba(GOLD,0.35)})` }} />
      <text x="34" y="38" textAnchor="middle" fontSize="14" fontWeight="800" fill="#f4efe5" style={{ fontVariantNumeric: 'tabular-nums' }}>%{yuzde}</text>
    </svg>
  );
}
...
function AkisGrafigi({ gunler }: { gunler: AkisGunu[] }) {
  ...
  return (
    <div className="rounded-xl px-3.5 py-2.5" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${rgba(MINT,0.16)}` }}>
      <div className="flex items-baseline justify-between gap-3">
        <span className="text-[10px] uppercase font-bold tracking-[.18em]" style={{ color: 'rgba(250,250,249,0.45)' }}>Son 7 gün belge akışı</span>
        <span className="text-[11px] font-bold tabular-nums" style={{ color: '#f4efe5' }}>{toplam.toLocaleString('tr-TR')}</span>
      </div>
      ...
      <div className="mt-1 flex items-center gap-3 text-[9.5px]" style={{ color: 'rgba(250,250,249,0.45)' }}>
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      >
        ...
        <span className="w-[86px] shrink-0 text-[9.5px] uppercase font-bold tracking-[.14em]" style={{ color: 'rgba(250,250,249,0.42)' }}>{k.kaynak}</span>
        <span className="min-w-0 flex-1 leading-snug">
          <span className="text-[13.5px] font-semibold" style={{ color: 'rgba(250,250,249,0.92)' }}>{k.baslik}</span>
          {k.aciklama && <span className="text-[12.5px]" style={{ color: 'rgba(250,250,249,0.48)' }}> — {k.aciklama}</span>}
        </span>
        {k.sayi != null && (
          <span className="shrink-0 whitespace-nowrap leading-none">
            <span className="text-[15px] font-bold tabular-nums" style={{ color: '#f4efe5' }}>{fmtSayi(k.sayi, k.sayiEtiket)}</span>
            {k.sayiEtiket && <span className="ml-1 text-[10px] font-semibold" style={{ color: 'rgba(250,250,249,0.42)' }}>{k.sayiEtiket}</span>}
          </span>
        )}
        {k.href && (
          <Link
            ...
            style={{ color: GOLD, background: `${rgba(GOLD,0.08)}`, border: `1px solid ${rgba(GOLD,0.2)}` }}
          >
            Aç <ArrowUpRight size={11} />
          </Link>
        )}
        <span className="w-4 shrink-0 flex justify-center">
          {acilir && <ChevronDown size={14} style={{ color: 'rgba(250,250,249,0.45)', transform: acik ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }} />}
        </span>
      </div>
      {acilir && acik && (
        <div className="pb-1.5" style={{ background: 'rgba(0,0,0,0.18)' }}>
          {(() => {
            ...
            const ic = (
              <div className="flex items-center gap-3 py-1.5 pr-4 transition hover:bg-white/[0.025]" style={{ paddingLeft: 122 }}>
                <span className="w-1 h-1 rounded-full shrink-0" style={{ background: 'rgba(250,250,249,0.3)' }} />
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[12.5px]" style={{ color: 'rgba(250,250,249,0.82)' }}>
                    {d.metin}{d.alt && <span style={{ color: 'rgba(250,250,249,0.42)' }}> · {d.alt}</span>}
                  </span>
                  {cubuk && (
                    <span className="mt-1 block h-[3px] rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.05)' }}>
                      <span className="block h-full rounded-full" style={{ width: `${oran}%`, background: `linear-gradient(90deg, ${MINT}, ${GOLD})`, opacity: 0.75 }} />
                    </span>
                  )}
                </span>
                {d.sayi != null && (
                  <span className="shrink-0 text-[12.5px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.85)' }}>
                    {fmtSayi(d.sayi, d.sayiEtiket)}{d.sayiEtiket && <span className="ml-1 text-[10px] font-semibold" style={{ color: 'rgba(250,250,249,0.42)' }}>{d.sayiEtiket}</span>}
                  </span>
                )}
                {d.href && <ArrowUpRight size={12} className="shrink-0 opacity-40" style={{ color: GOLD }} />}
...
function BolumBandi({ k, adet }: { k: BolumKey; adet: number }) {
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(90deg, ${b.renk}14, transparent 55%)`, borderTop: `1px solid ${b.renk}33` }}
    >
      <span className="w-2 h-2 rounded-full" style={{ background: b.renk, boxShadow: `0 0 10px ${b.renk}88` }} />
      <span className="text-[10.5px] uppercase font-bold tracking-[.24em]" style={{ color: b.renk }}>{b.baslik}</span>
      <span className="text-[10.5px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.45)' }}>{adet}</span>
    </div>
  );
}
...
export function BugunListesi({ hitap }: { hitap?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pt-5 pb-4 flex items-center gap-5 flex-wrap" style={{ borderBottom: `1px solid ${rgba(MINT,0.10)}`, background: 'linear-gradient(180deg, rgba(255,255,255,0.025), transparent)' }}>
        <div className="min-w-0 flex-1">
          <div className="text-[24px] font-semibold leading-tight tracking-tight" style={{ color: P.metin }}>
            {selam(saat)}{hitap ? `, ${hitap}` : ''}.
          </div>
          <div className="mt-1 text-[13.5px] italic" style={{ color: GOLD, opacity: 0.85 }}>{motivasyon()}</div>
          <div className="mt-1.5 text-[12px]" style={{ color: 'rgba(250,250,249,0.5)' }}>{baglam}</div>
        </div>
        {!!data?.akis?.length && <AkisGrafigi gunler={data.akis} />}
        {ay && (
          <div className="flex items-center gap-3 rounded-xl px-3.5 py-2.5" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${rgba(GOLD,0.16)}` }}>
            <Halka yuzde={ay.yuzde} />
            <div className="leading-tight">
              <div className="text-[10px] uppercase font-bold tracking-[.18em]" style={{ color: 'rgba(250,250,249,0.45)' }}>{ay.ad} ilerlemesi</div>
              <div className="mt-1 text-[13.5px] font-semibold tabular-nums" style={{ color: '#f4efe5' }}>{ay.tamam} / {ay.toplam} <span className="font-normal" style={{ color: 'rgba(250,250,249,0.55)' }}>mükellef tamamlandı</span></div>
              <div className="text-[11.5px] tabular-nums" style={{ color: 'rgba(250,250,249,0.5)' }}>ay sonuna {ay.isGunuKaldi} iş günü</div>
            </div>
          </div>
        )}
        <div className="flex items-center gap-2 self-start">
          ...
          <button
            ...
            style={{ background: `${rgba(MINT,0.055)}`, border: `1px solid ${rgba(MINT,0.14)}`, color: `${rgba(MINT,0.72)}` }}
          >
      ...
      {/* Liste */}
      {isLoading ? (
        <div className="flex items-center gap-2 text-[13px] px-6 py-5" style={{ color: 'rgba(250,250,249,0.5)' }}>
          ...
          return (
            <div key={key}>
              ...
              {satirlar.length ? satirlar.map((k) => <KonuSatiri key={k.id} k={k} />) : (
                <div className="px-4 py-2.5 text-[12.5px]" style={{ borderTop: '1px solid rgba(255,255,255,0.06)', color: 'rgba(250,250,249,0.4)', paddingLeft: 122 }}>{bolum(P)[key].bos}</div>
              )}
            </div>
          );
        })

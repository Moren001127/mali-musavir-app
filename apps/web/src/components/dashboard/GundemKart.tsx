...
function Degisim({ yuzde, kurMu }: { yuzde: number | null; kurMu: boolean }) {
  ...
  return (
    <span className="text-[10.5px] font-semibold tabular-nums inline-flex items-center gap-0.5" style={{ color: renk }}>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.016)', border: '1px solid rgba(143,215,189,0.10)' }}>
      <div className="flex items-center justify-between gap-2 px-3.5 py-2">
        <span className="text-[10px] uppercase font-bold tracking-[.18em]" style={{ color: 'rgba(221,246,238,0.62)' }}>{baslik}</span>
        {sag}
      </div>
      {children}
    </div>
...
export function GundemKart() {
  ...
  return (
    ...
    >
      ...
      {/* Başlık */}
      <div className="px-4 pt-4 pb-3 flex items-center justify-between gap-2 flex-wrap" style={{ borderBottom: '1px solid rgba(216,189,134,0.10)' }}>
        <div className="flex items-center gap-2 min-w-0">
          <ScrollText size={14} style={{ color: GOLD }} />
          <span className="text-[10px] uppercase font-bold tracking-[.22em]" style={{ color: 'rgba(244,236,220,0.75)' }}>Başvuru Sayıları</span>
        </div>
        <div className="flex items-center gap-2">
          {data?.kurTarihi && (
            <span className="text-[10.5px] tabular-nums" style={{ color: 'rgba(250,250,249,0.4)' }} title="TCMB kur tarihi">{data.kurTarihi}</span>
          )}
          <button
            ...
            style={{ background: 'rgba(216,189,134,0.06)', border: '1px solid rgba(216,189,134,0.16)', color: 'rgba(244,236,220,0.75)' }}
          >
      ...
      <div className="px-4 pt-3 pb-4 space-y-3">
        {isLoading && (
          <div className="flex items-center gap-2 text-[13px] py-1" style={{ color: 'rgba(250,250,249,0.5)' }}>
        ...
        {!isLoading && (data?.kurlar?.length || piyasa.length) ? (
          <Bolum baslik="Kur · Altın" sag={<span className="text-[10px]" style={{ color: 'rgba(250,250,249,0.38)' }}>TCMB satış</span>}>
            {(data?.kurlar ?? []).map((k) => (
              <div key={k.kod} className="flex items-center gap-2 px-3.5 py-1.5" style={SATIR_KENAR}>
                <span className="w-9 text-[10px] font-black tracking-wider" style={{ color: GOLD }}>{k.kod}</span>
                <span className="flex-1 text-[13.5px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.9)' }}>{fmtKur(k.satis)}</span>
                <Degisim yuzde={k.degisimYuzde} kurMu />
              </div>
            ))}
            {piyasa.map((p) => (
              <div key={p.kod} className="flex items-center gap-2 px-3.5 py-1.5" style={SATIR_KENAR} title={p.isim}>
                <span className="w-9 text-[10px] font-black tracking-wider" style={{ color: GOLD }}>ALTIN</span>
                <span className="flex-1 text-[13.5px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.9)' }}>
                  {fmtPiyasa(p.deger, p.ondalik)}
                  {p.birim && <span className="text-[10px] font-semibold ml-1" style={{ color: 'rgba(250,250,249,0.42)' }}>{p.birim}</span>}
                </span>
        ...
        {data?.enflasyon && (
          ...
          >
            <div className="flex items-center gap-x-4 gap-y-1 flex-wrap px-3.5 py-2" style={SATIR_KENAR}>
              {data.enflasyon.aylik != null && (
                <span className="inline-flex items-baseline gap-1.5">
                  <span className="text-[10.5px]" style={{ color: 'rgba(250,250,249,0.45)' }}>Aylık</span>
                  <span className="text-[13px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.88)' }}>%{data.enflasyon.aylik.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
                </span>
              )}
              {data.enflasyon.yillik != null && (
                <span className="inline-flex items-baseline gap-1.5">
                  <span className="text-[10.5px]" style={{ color: 'rgba(250,250,249,0.45)' }}>Yıllık</span>
                  <span className="text-[13px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.88)' }}>%{data.enflasyon.yillik.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
                </span>
              )}
            </div>
            {data.enflasyon.kiraArtisTavani != null && (
              <div className="flex items-center gap-2 px-3.5 py-2" style={{ ...SATIR_KENAR, background: 'rgba(216,189,134,0.05)' }}>
                <Home size={11} style={{ color: GOLD }} />
                <span className="flex-1 text-[12px]" style={{ color: 'rgba(250,250,249,0.7)' }}>Kira artış tavanı</span>
                <span className="text-[14px] font-bold tabular-nums" style={{ color: GOLD }}>%{data.enflasyon.kiraArtisTavani.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
              </div>
            )}
          </Bolum>
        )}
        ...
        {!!data?.sabitler?.length && (
          <Bolum baslik="Oran ve Tutarlar" sag={<span className="text-[10px]" style={{ color: 'rgba(250,250,249,0.38)' }}>kaynak için tıkla</span>}>
            {data.sabitler.map((s) => (
              <a
                key={s.kod}
                href={s.kaynakUrl}
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className="block text-[12.5px] font-semibold leading-tight" style={{ color: 'rgba(250,250,249,0.86)' }}>{s.etiket}</span>
                  <span className="block truncate text-[10.5px] mt-0.5" style={{ color: 'rgba(250,250,249,0.42)' }}>{s.alt ? `${s.alt} · ` : ''}{s.gecerlilik}</span>
                </span>
                <span className="shrink-0 text-[13.5px] font-bold tabular-nums" style={{ color: 'rgba(250,250,249,0.92)' }}>{s.deger}</span>
                <ExternalLink size={11} className="shrink-0 opacity-25 transition group-hover:opacity-90" style={{ color: GOLD }} />
              </a>
            ))}
          </Bolum>
        )}
        ...
        {mevzuat.length > 0 && (
          <Bolum baslik="Resmî Gazete · Bugün">
            {mevzuat.map((m, i) => {
              ...
              return (
                <a key={i} href={m.url} target="_blank" rel="noopener noreferrer" className="group flex items-start gap-2.5 px-3.5 py-2 transition hover:bg-white/[0.03]" style={SATIR_KENAR}>
                  <span className="mt-[6px] w-1.5 h-1.5 rounded-full shrink-0" style={{ background: yuksek ? GOLD : MINT }} />
                  <span className="min-w-0 flex-1">
                    <span className="block text-[12.5px] font-semibold leading-snug" style={{ color: 'rgba(250,250,249,0.9)' }}>{m.baslik}</span>
                    {m.neden && <span className="block mt-0.5 text-[11px] leading-snug" style={{ color: 'rgba(250,250,249,0.5)' }}>{m.neden}</span>}
                  </span>
                  <ExternalLink size={12} className="shrink-0 mt-1 opacity-40 transition group-hover:opacity-90" style={{ color: yuksek ? GOLD : MINT }} />
                </a>
              );
            })}
          </Bolum>
        ...
        {!isLoading && data && (
          <p className="flex items-center gap-1.5 text-[10.5px] px-1" style={{ color: 'rgba(250,250,249,0.38)' }}>
            {data.mevzuatHazirlaniyor ? (
              <><Loader2 size={10} className="animate-spin" style={{ color: MINT }} /> Resmî Gazete taranıyor…</>
            ) : mevzuat.length === 0 ? (
              <><Landmark size={10} style={{ color: MINT }} /> Resmî Gazete: ilgili yayın yok{data.mevzuatToplam ? ` · ${data.mevzuatToplam} madde tarandı` : ''}</>
            ) : null}

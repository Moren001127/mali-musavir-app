  ...
  return (
    <>
      {!open ? (
        ...
        >
          ...
          {unreadTotal > 0 ? (
            <span
              ...
              style={portalStyle({ background: '#f43f5e', color: '#fff' })}
            >
        ...
        >
          <aside className="flex min-w-0 flex-col border-r" style={portalStyle({ borderColor: LINE })}>
            <div className="flex items-center gap-3 border-b px-3 py-3" style={portalStyle({ borderColor: LINE })}>
              <div
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"
                style={portalStyle({ background: 'rgba(143,216,194,0.12)', border: '1px solid rgba(143,216,194,0.28)', color: CHAT })}
              >
                <Users size={17} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold" style={portalStyle({ color: TEXT })}>Ofis Chat</p>
                <p className="truncate text-[11px]" style={portalStyle({ color: MUTED })}>
                  {unreadTotal > 0 ? `${unreadTotal} okunmamis` : 'Ekip ici mesajlasma'}
                </p>
              </div>
            </div>

            <div className="border-b p-2" style={portalStyle({ borderColor: LINE })}>
              <div className="flex items-center gap-2 rounded-lg border px-2" style={portalStyle({ borderColor: CHAT_LINE, background: 'rgba(255,255,255,0.025)' })}>
                <Search size={13} style={portalStyle({ color: MUTED })} />
                <input
                  value={search}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-2">
              <p className="mb-1 px-1 text-[10px] font-bold uppercase" style={portalStyle({ color: 'rgba(212,184,118,0.72)', letterSpacing: 0 })}>
                Kanallar
              </p>
              ...
              <p className="mb-1 mt-4 px-1 text-[10px] font-bold uppercase" style={portalStyle({ color: 'rgba(212,184,118,0.72)', letterSpacing: 0 })}>
                Kisiler
              </p>
              <div className="space-y-1">
                {usersQuery.isLoading ? (
                  <div className="flex items-center gap-2 px-2 py-2 text-[11px]" style={portalStyle({ color: MUTED })}>
                    <Loader2 size={12} className="animate-spin" /> Yukleniyor
                  </div>
                ) : users.length === 0 ? (
                  <p className="px-2 py-2 text-[11px]" style={portalStyle({ color: MUTED })}>Kisi bulunmadi.</p>
                ) : (
                  users.map((user) => (
                    <button
                      key={user.id}
                      ...
                      style={portalStyle({ borderColor: 'rgba(255,255,255,0.04)', color: TEXT })}
                    >
          ...
          <section className="flex min-w-0 flex-col">
            <div className="flex items-center gap-3 border-b px-4 py-3" style={portalStyle({ borderColor: LINE })}>
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold" style={portalStyle({ color: TEXT })}>
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4">
              {threadQuery.isLoading || threadsQuery.isLoading ? (
                ...
                <div className="rounded-lg border px-3 py-3 text-[12.5px]" style={portalStyle({ borderColor: LINE, color: MUTED })}>
                  ...
                  return (
                    <div key={message.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`flex max-w-[82%] gap-2 ${isMine ? 'flex-row-reverse' : ''}`}>
                        ...
                        <div>
                          {!isMine ? (
                            <p className="mb-1 text-[10.5px] font-semibold" style={portalStyle({ color: 'rgba(212,184,118,0.76)' })}>
                          ...
                          <p className={`mt-1 text-[10px] ${isMine ? 'text-right' : ''}`} style={portalStyle({ color: 'rgba(250,250,249,0.32)' })}>
  ...
  return (
    ...
    >
      <span
        ...
        style={portalStyle({ background: 'rgba(212,184,118,0.10)', color: GOLD, border: '1px solid rgba(212,184,118,0.22)' })}
      >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-[10.5px]" style={portalStyle({ color: MUTED })}>
          {thread.lastMessage?.content || 'Henuz mesaj yok'}
        </span>
      </span>
      {thread.unreadCount > 0 ? (
        <span className="rounded-full px-1.5 py-0.5 text-[10px] font-bold" style={portalStyle({ background: '#f43f5e', color: '#fff' })}>

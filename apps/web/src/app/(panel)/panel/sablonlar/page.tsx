  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5 pb-12">
      ...
      <header className="relative overflow-hidden rounded-2xl border p-5" style={portalStyle({
        ...
        <div className="absolute inset-x-0 top-0 h-1" style={portalStyle({ background: 'linear-gradient(90deg, #34d399, #22d3ee, #60a5fa, #a855f7, #f472b6, #fb923c, #d4b876)' })} />
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          <h1 className="flex items-center gap-2.5 text-[28px] font-semibold leading-tight" style={portalStyle({ color: TEXT })}>
            <span className="grid h-10 w-10 place-items-center rounded-xl" style={portalStyle({ background: 'linear-gradient(135deg, #34d399, #22d3ee)', boxShadow: '0 6px 18px rgba(52,211,153,0.35)' })}>
              <MessageSquareText size={22} style={portalStyle({ color: '#0a1410' })} />
            </span>
            Mesaj Şablonları
          </h1>
          <div className="flex items-center gap-2">
            <button onClick={exportJson} title="Tüm şablonları JSON yedekle" className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-2 text-[12px] font-medium" style={portalStyle({ borderColor: LINE, color: MUTED, background: 'rgba(255,255,255,0.03)' })}>
              <Download size={13} /> Dışa
            </button>
            <button onClick={() => fileRef.current?.click()} title="JSON'dan şablon içe aktar" className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-2 text-[12px] font-medium" style={portalStyle({ borderColor: LINE, color: MUTED, background: 'rgba(255,255,255,0.03)' })}>
              <Upload size={13} /> İçe
            </button>
        ...
        <p className="mt-2 max-w-2xl text-[13px]" style={portalStyle({ color: MUTED })}>
          WhatsApp ve e-posta için kendi şablonlarını tanımla. <span style={portalStyle({ color: CYAN })}>{'{ad}'}</span>, <span style={portalStyle({ color: CYAN })}>{'{tutar}'}</span> gibi alanlar gönderimde mükellef verisiyle otomatik dolar.
        </p>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <div className="inline-flex items-center gap-1.5 rounded-xl border px-2 py-1.5 text-[12.5px] font-semibold" style={portalStyle({ borderColor: LINE, color: MUTED, background: 'rgba(255,255,255,0.03)' })}>
          <ArrowDownUp size={14} />
        ...
        {KATEGORILER.filter((c) => catCounts[c.key]).map((c) => {
          ...
          return (
            <button key={c.key} onClick={() => setCatFilter(on ? 'all' : c.key)} className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12px] font-semibold transition-colors"
              ...
              <span className="h-2 w-2 rounded-full" style={portalStyle({ background: c.color })} />
      ...
      {/* GALERİ */}
      {loading ? (
        <div className="grid place-items-center rounded-2xl border py-20 text-[13px]" style={portalStyle({ borderColor: LINE, color: MUTED, background: '#0f0d0b' })}>
        ...
        <div className="grid gap-3.5" style={portalStyle({ gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))' })}>
          {filtered.map((t) => {
            ...
            return (
              <div key={t.id} onClick={() => openEditor(t)}
                ...
                <span className="absolute inset-y-0 left-0 w-[3px]" style={portalStyle({ background: c.color })} />

                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="inline-flex items-center gap-1.5 text-[11.5px] font-bold" style={portalStyle({ color: c.color })}>
                    <span className="h-2.5 w-2.5 rounded-full" style={portalStyle({ background: c.color, boxShadow: `0 0 8px ${c.color}` })} />
                    {c.label}
                  </span>
                  <div className="flex items-center gap-2" style={portalStyle({ color: MUTED })}>
                ...
                <h3 className="truncate text-[15px] font-semibold" style={portalStyle({ color: TEXT })}>{t.ad}</h3>
                <p className="mt-1.5 text-[12.5px]" style={portalStyle({ color: MUTED, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', minHeight: 38 })}>
                ...
                <div className="mt-3 flex items-center justify-between gap-2 border-t pt-2.5" style={portalStyle({ borderColor: LINE })}>
                  ...
                  <span className="inline-flex items-center gap-1 whitespace-nowrap text-[11px]" style={portalStyle({ color: MUTED2 })}>
          ...
          {!filtered.length && (
            <div className="col-span-full rounded-2xl border p-8 text-center text-[13px]" style={portalStyle({ borderColor: LINE, color: MUTED, background: '#0f0d0b' })}>
      ...
      {dndEnabled && filtered.length > 1 && (
        <p className="text-[11px]" style={portalStyle({ color: MUTED2 })}>İpucu: Kartları sürükleyerek sıralayabilirsin. (Filtre/arama açıkken sürükleme kapalıdır.)</p>
      )}
...
function Badge({ color, icon, children }: { color: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10.5px] font-bold" style={portalStyle({ color, background: `${color}1f` })}>
      {icon}{children}
    </span>
  );
}
...
function Toggle({ on, color, icon, label, onClick }: { on: boolean; color: string; icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-[12.5px] font-semibold transition-colors"
      ...
      <span className="relative inline-block h-[18px] w-[32px] rounded-full transition-colors" style={portalStyle({ background: on ? `${color}88` : 'rgba(255,255,255,0.14)' })}>
        <span className="absolute top-[2px] h-[14px] w-[14px] rounded-full transition-all" style={portalStyle({ left: on ? 16 : 2, background: on ? '#0a1410' : '#9ca3af' })} />
      </span>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-[min(960px,96vw)] overflow-hidden border-l shadow-2xl"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-[16px] font-bold" style={portalStyle({ color: TEXT })}>
              <MessageSquareText size={18} style={portalStyle({ color: ACCENT })} />
              {isNew ? 'Yeni Şablon' : 'Şablon Düzenle'}
              {p.dirty && <span className="inline-flex items-center gap-1 text-[11px] font-medium" style={portalStyle({ color: AMBER })}><CircleDot size={11} /> kaydedilmedi</span>}
            </div>
            <button onClick={p.onClose} className="grid h-8 w-8 place-items-center rounded-lg border" style={portalStyle({ borderColor: LINE, color: MUTED, background: 'rgba(255,255,255,0.03)' })}><X size={16} /></button>
          </div>

          <label className={labelCls} style={portalStyle({ color: 'rgba(52,211,153,0.85)' })}>Şablon adı</label>
          <input className="w-full rounded-lg border px-3 py-2 text-[14px] font-semibold" style={portalStyle(fieldStyle)} value={draft.ad} onChange={(e) => p.onPatch({ ad: e.target.value })} />

          {/* Kanal */}
          <div className="mt-4">
            <label className={labelCls} style={portalStyle({ color: MUTED })}>Kanal</label>
            <div className="flex gap-1.5 rounded-xl border p-1" style={portalStyle({ borderColor: LINE, background: 'rgba(255,255,255,0.03)' })}>
          ...
          {/* Kategori */}
          <div className="mt-3">
            <label className={labelCls} style={portalStyle({ color: MUTED })}>Kategori</label>
            <div className="flex flex-wrap gap-1.5">
              {KATEGORILER.map((k) => {
                ...
                return (
                  <button key={k.key} type="button" onClick={() => p.onPatch({ kategori: k.key })}
                    ...
                    <span className="h-2 w-2 rounded-full" style={portalStyle({ background: k.color })} /> {k.label}
                  </button>
                );
              })}
            </div>
          ...
          {draft.kanal !== 'WHATSAPP' && (
            <div className="mt-3">
              <label className={labelCls} style={portalStyle({ color: MUTED })}>E-posta konusu</label>
              <input className="w-full rounded-lg border px-3 py-2 text-[14px]" style={portalStyle(fieldStyle)} value={draft.emailSubject || ''} onChange={(e) => p.onPatch({ emailSubject: e.target.value })} placeholder="Örn. Evrak Talebi - {dönem}" />
            </div>
          )}

          {/* Mesaj metni + alan ekle */}
          <div className="mt-3 overflow-hidden rounded-xl border" style={portalStyle({ borderColor: LINE, background: 'rgba(255,255,255,0.02)' })}>
            <div className="flex items-center justify-between border-b px-3 py-2" style={portalStyle({ borderColor: LINE })}>
              <span className="text-[11px] font-medium uppercase tracking-wider" style={portalStyle({ color: MUTED })}>Mesaj metni</span>
              <span className="text-[11px] tabular-nums" style={portalStyle({ color: MUTED })}>{draft.body.length} karakter</span>
            </div>
            ...
            <div className="border-t px-3 py-2.5" style={portalStyle({ borderColor: LINE, background: 'rgba(34,211,238,0.03)' })}>
              <div className="mb-1.5 text-[11px] font-medium uppercase tracking-wider" style={portalStyle({ color: MUTED })}>Alan ekle — tıklayınca metne eklenir</div>
              <div className="flex flex-wrap gap-1.5">
                {PLACEHOLDERS.map((ph) => {
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                <span className="text-[11px]" style={portalStyle({ color: MUTED })}>Hazır parça:</span>
                {([['Selamlama', 'Sayın {ad},\n\n'], ['Kapanış', '\n\nSaygılarımızla,'], ['İmza', '\nMoren Mali Müşavirlik'], ['Teşekkür', '\n\nTeşekkür ederiz.']] as const).map(([lbl, txt]) => (
                  <button key={lbl} type="button" onClick={() => p.onInsertText(txt)} className="rounded-md border px-2 py-0.5 text-[11.5px] font-medium transition-colors" style={portalStyle({ borderColor: LINE, color: MUTED, background: 'rgba(255,255,255,0.03)' })}>{lbl}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          {/* AI Asistan */}
          <div className="mt-3 overflow-hidden rounded-xl border" style={portalStyle({ borderColor: 'rgba(168,85,247,0.3)', background: 'rgba(168,85,247,0.06)' })}>
            <div className="flex items-center gap-1.5 border-b px-3 py-2 text-[11px] font-semibold uppercase tracking-wider" style={portalStyle({ borderColor: 'rgba(168,85,247,0.2)', color: '#c4a3f0' })}>
            ...
            <div className="space-y-2.5 p-3">
              <div className="flex flex-wrap gap-1.5">
                {([['Kısalt', 'daha kısa ve öz yap'], ['Resmileştir', 'daha resmi ve kurumsal yap'], ['Samimileştir', 'daha sıcak ve samimi yap'], ['Kibarlaştır', 'daha kibar ve nazik yap'], ['Dili düzelt', 'yazım ve dil bilgisi hatalarını düzelt']] as const).map(([lbl, ins]) => (
                  <button key={lbl} type="button" disabled={p.aiBusy} onClick={() => p.onAiImprove(ins)} className="rounded-md border px-2.5 py-1 text-[12px] font-medium transition-colors disabled:opacity-50" style={portalStyle({ borderColor: 'rgba(168,85,247,0.3)', color: '#c4a3f0', background: 'rgba(168,85,247,0.08)' })}>{lbl}</button>
                ))}
              </div>
              <div className="flex gap-1.5">
                ...
                <button type="button" disabled={p.aiBusy} onClick={p.onAiGenerate} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-semibold disabled:opacity-50" style={portalStyle({ background: 'linear-gradient(135deg,#a855f7,#7c3aed)', color: '#fff' })}>
                  <Wand2 size={14} /> Yaz
                </button>
              </div>
              <div className="text-[11px]" style={portalStyle({ color: MUTED })}>AI metni öneri olarak uygular; dilediğin gibi düzenleyebilirsin. (Max aboneliğinden — ek ücret yok.)</div>
            </div>
          </div>
          ...
          {draft.auto && (
            <div className="mt-2.5 rounded-lg border px-3 py-2 text-[12px]" style={portalStyle({ borderColor: 'rgba(96,165,250,0.3)', background: 'rgba(96,165,250,0.07)', color: '#9cc4f5' })}>
          ...
          {/* TEST GÖNDER */}
          <div className="mt-3 overflow-hidden rounded-xl border" style={portalStyle({ borderColor: 'rgba(52,211,153,0.28)', background: 'rgba(52,211,153,0.05)' })}>
            <div className="flex items-center gap-1.5 border-b px-3 py-2 text-[11px] font-semibold uppercase tracking-wider" style={portalStyle({ borderColor: 'rgba(52,211,153,0.2)', color: ACCENT })}>
              <Send size={13} /> Test gönder — örnek veriyle
            </div>
            <div className="space-y-2.5 p-3">
              <div className="flex gap-1.5 rounded-lg border p-1" style={portalStyle({ borderColor: LINE, background: 'rgba(255,255,255,0.03)', maxWidth: 280 })}>
              ...
              <div className="flex gap-1.5">
                <input id="test-target-input" value={p.testTarget} onChange={(e) => p.setTestTarget(e.target.value)}
                  ...
                  className="flex-1 rounded-lg border px-3 py-2 text-[13px]" style={portalStyle(fieldStyle)} />
                <button type="button" disabled={p.testBusy || isNew} onClick={p.onSendTest} className="inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-[13px] font-semibold disabled:opacity-50" style={portalStyle({ background: 'linear-gradient(135deg,#34d399,#22d3ee)', color: '#0a1410' })}>
                  {p.testBusy ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} Gönder
                </button>
              </div>
              <div className="text-[11px]" style={portalStyle({ color: MUTED2 })}>
          ...
          {/* Eylemler */}
          <div className="mt-5 flex flex-wrap gap-2.5 border-t pt-4" style={portalStyle({ borderColor: LINE })}>
        ...
        <div className="hidden w-[340px] flex-shrink-0 overflow-y-auto border-l p-4 lg:block" style={portalStyle({ borderColor: LINE, background: '#0b0a08' })}>
          <div className="mb-3 flex items-center justify-between">
            <span className="text-[11px] font-medium uppercase tracking-wider" style={portalStyle({ color: MUTED })}>Canlı önizleme</span>
            {draft.kanal === 'BOTH' && (
              <div className="flex rounded-lg border p-0.5" style={portalStyle({ borderColor: LINE })}>
                {(['WHATSAPP', 'EMAIL'] as const).map((k) => (
          ...
          {p.effPreviewKanal === 'WHATSAPP' ? (
            <div className="rounded-xl p-3" style={portalStyle({ background: 'linear-gradient(180deg,#0c1a14,#0b141a)' })}>
              <div className="mb-2 flex items-center gap-2">
                <span className="grid h-7 w-7 place-items-center rounded-full text-[12px] font-bold" style={portalStyle({ background: '#25d366', color: '#0a1410' })}>M</span>
                <div>
                  <div className="text-[12px] font-semibold" style={portalStyle({ color: '#e9edef' })}>{OFFICE}</div>
                  <div className="text-[10px]" style={portalStyle({ color: '#8aa9a0' })}>çevrimiçi</div>
                </div>
              </div>
              <div className="ml-auto max-w-[94%] rounded-[10px] rounded-br-[2px] px-3 py-2.5 text-[13.5px]" style={portalStyle({ background: '#005c4b', color: '#e9edef', whiteSpace: 'pre-wrap', lineHeight: 1.45 })}>
                {renderPreview(draft.body, 'WHATSAPP')}
                {draft.attachPdf && <div className="mt-2 inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-[12px]" style={portalStyle({ background: '#0b141a', color: '#8aa9a0' })}><Paperclip size={12} /> Belge.pdf</div>}
                <div className="mt-1 flex items-center justify-end gap-1 text-[10px]" style={portalStyle({ color: '#8aa9a0' })}>
                  09:41 <Check size={11} style={portalStyle({ color: '#53bdeb' })} className="-mr-1.5" /><Check size={11} style={portalStyle({ color: '#53bdeb' })} />
                </div>
              </div>
            </div>
          ) : (
            <div className="overflow-hidden rounded-xl" style={portalStyle({ background: '#fff', color: '#1b2230' })}>
              <div className="px-3.5 py-2.5 text-[13px] font-bold text-white" style={portalStyle({ background: 'linear-gradient(135deg,#1b3a6b,#2a5298)' })}>{OFFICE}</div>
          ...
          {/* İstatistik */}
          <div className="mt-4 rounded-xl border p-3" style={portalStyle({ borderColor: LINE, background: 'rgba(255,255,255,0.02)' })}>
            <div className="mb-2.5 text-[11px] font-medium uppercase tracking-wider" style={portalStyle({ color: MUTED })}>Bu şablonun kullanımı</div>
            <div className="grid grid-cols-2 gap-2.5">
              <div className="rounded-lg border p-2.5" style={portalStyle({ borderColor: LINE })}>
                <div className="text-[19px] font-extrabold tabular-nums" style={portalStyle({ color: ACCENT })}>{draft.kullanimSayisi ?? 0}</div>
                <div className="mt-0.5 text-[10.5px]" style={portalStyle({ color: MUTED })}>toplam gönderim</div>
              </div>
              <div className="rounded-lg border p-2.5" style={portalStyle({ borderColor: LINE })}>
                <div className="inline-flex items-center gap-1 text-[14px] font-bold" style={portalStyle({ color: TEXT })}><Clock size={13} style={portalStyle({ color: MUTED })} /> {relTime(draft.sonKullanim)}</div>
                <div className="mt-0.5 text-[10.5px]" style={portalStyle({ color: MUTED })}>son kullanım</div>
              </div>
            </div>
          </div>
        </div>

...
/* Resmi WhatsApp logosu (Fatura Merkezi ile aynı ikon) */
...
function KarsilastirmaTablosu({
  ...
  const [waDonem, setWaDonem] = useState<number | null>(null);
  const [waLoading, setWaLoading] = useState(false);
  const [waPreview, setWaPreview] = useState<{ telefon: string; mesaj: string } | null>(null);
  const [waPhone, setWaPhone] = useState('');

  const waOnizle = async (donem: number) => {
    const c = yilData?.ceyrekler?.[donem - 1];
    if (!c) return;
    setWaDonem(donem);
    setWaLoading(true);
    try {
      const r = await isletmeHesapOzetiApi.whatsappBilgi(c.id, { dryRun: true });
      setWaPreview({ telefon: r.telefon || '', mesaj: r.mesaj || '' });
      setWaPhone(r.telefon || '');
      setWaOpen(true);
    } catch (e: any) {
      toast.error(e?.response?.data?.message || 'Önizleme hazırlanamadı');
    } finally {
      setWaLoading(false);
    }
  };
  ...
  const waGonder = async () => {
    if (waDonem == null) return;
    const c = yilData?.ceyrekler?.[waDonem - 1];
    if (!c) return;
    setWaLoading(true);
    try {
      await isletmeHesapOzetiApi.whatsappBilgi(c.id, { dryRun: false, telefon: waPhone.trim() });
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-bold text-stone-100">
                WhatsApp Bilgilendirme{waDonem ? ` — ${DONEM_ROMAN[waDonem]}. Dönem` : ''}
              </h3>
            </div>

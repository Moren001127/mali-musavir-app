...
export function TopluSerit({ secili, onTemizle, onIslem }: { secili: string[]; onTemizle: () => void; onIslem: (dto: Omit<TopluInput, 'ids'>) => Promise<unknown> }) {
  ...
  return (
    ...
    <div className="pointer-events-none sticky bottom-[84px] z-[41] flex justify-center px-1 lg:bottom-3">
      ...
      >
        <span className="inline-flex items-center gap-1.5 pr-1 text-[12.5px] font-bold" style={{ color: METIN }}>
          <span className="rounded-md px-1.5 text-[11px] font-extrabold tabular-nums leading-5" style={{ background: GOLD, color: '#0f0d0b' }}>
            {secili.length}
          </span>
          seçili
        </span>
        ...
        <button type="button" disabled={!!calisan} onClick={() => calistir('tamamla', { islem: 'tamamla' })} className={dugme} style={{ background: `${YESIL}1f`, color: YESIL, border: `1px solid ${YESIL}55` }} title="Seçilenleri tamamla">
          {calisan === 'tamamla' ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} Tamamla
        </button>

        <AcilirMenu genislik={240} tetik={({ ref, ac }) => (
          <button ref={ref} type="button" disabled={!!calisan} onClick={ac} className={dugme} style={{ background: `${MOR}1f`, color: '#c084fc', border: `1px solid ${MOR}55` }} title="Seçilenleri ertele">
        ...
        <AcilirMenu genislik={230} tetik={({ ref, ac }) => (
          <button ref={ref} type="button" disabled={!!calisan} onClick={ac} className={dugme} style={{ background: 'rgba(255,255,255,0.05)', color: METIN, border: '1px solid rgba(255,255,255,0.12)' }} title="Seçilenlerin kategorisini değiştir">
          ...
          {(kapat) => (
            <div className="py-1">
              ...
              {CATEGORY_OPTIONS.map((c) => (
                <MenuSatiri key={c.value} ikon={<span className="h-2 w-2 rounded-full" style={{ background: c.color }} />} onClick={() => { kapat(); calistir('kategori', { islem: 'kategori', category: c.value }); }}>
                  {c.label}
                </MenuSatiri>
              ))}
              <MenuAyrac />
        ...
        <AcilirMenu genislik={200} tetik={({ ref, ac }) => (
          <button ref={ref} type="button" disabled={!!calisan} onClick={ac} className={dugme} style={{ background: 'rgba(255,255,255,0.05)', color: METIN, border: '1px solid rgba(255,255,255,0.12)' }} title="Seçilenlerin önceliğini değiştir">
        ...
        <button
          ...
          className={dugme}
          style={{ background: `${KIRMIZI}1a`, color: '#fca5a5', border: `1px solid ${KIRMIZI}55` }}
          title="Seçilenleri sil"
        >
          {calisan === 'sil' ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />} Sil
        </button>

        <button type="button" onClick={onTemizle} className="inline-flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-white/10" style={{ color: IKINCIL }} title="Seçimi bırak">
          <X size={14} />
        </button>
      </div>
    </div>

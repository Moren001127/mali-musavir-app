  ...
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
      {SUTUNLAR.map((s) => {
        ...
        return (
          <section key={s.key} data-gorev-sutun={s.key} className="min-w-0 overflow-hidden rounded-2xl" style={portalStyle({ ...kartArkaPlan(s.renk), minHeight: 240 })}>
            <div data-gorev-sutun-baslik className="flex items-center gap-2 px-3.5 py-2.5" style={portalStyle({ background: 'rgba(255,255,255,0.045)', borderBottom: `1px solid ${KENAR_NOTR}` })}>
              <span data-gorev-sutun-nokta className="h-1.5 w-1.5 rounded-full" style={portalStyle({ background: s.renk, opacity: 0.8 })} />
              <h3 className="text-[11.5px] font-semibold uppercase tracking-[.08em]" style={portalStyle({ color: 'rgba(250,250,249,0.85)' })}>
                {s.ad}
              </h3>
              <span data-gorev-sutun-sayi className="ml-auto text-[11.5px] tabular-nums" style={portalStyle({ color: IKINCIL })}>
                {liste.length}
              </span>
            </div>
            <div data-gorev-sutun-govde className="space-y-2 p-2">
              {s.key === 'DONE' && bitenlerYukleniyor && liste.length === 0 && (
                <div data-gorev-soluk className="flex items-center justify-center gap-2 py-6 text-[12px]" style={portalStyle({ color: IKINCIL })}>
                  <Loader2 size={13} className="animate-spin" /> Bitenler alınıyor
                </div>
              )}
              {liste.length === 0 && !(s.key === 'DONE' && bitenlerYukleniyor) && (
                <div data-gorev-bos>
                  <BosDurum ikon={<Check size={16} />} metin={`${s.ad} sütununda görev yok`} renk={s.renk} />
                </div>
              )}
              {liste.map((t) => (
...
function KanbanKarti({ gorev: t, sutun, eylemler, acik }: { gorev: Task; sutun: TaskStatus; eylemler: GorevEylemleri; acik?: boolean }) {
  ...
  return (
    <article data-gorev-kart data-oncelik={t.priority} data-acik={!!acik} className="rounded-xl p-3" style={portalStyle({ background: 'rgba(0,0,0,0.30)', border: `1px solid ${acik ? `${GOLD}99` : KENAR_NOTR}` })}>
      <button type="button" data-gorev-satir-baslik onClick={() => eylemler.ac(t.id)} title="Detayı aç" className={`block w-full text-left text-[13px] font-medium leading-5 hover:underline decoration-dotted underline-offset-4 ${t.status === 'DONE' ? 'line-through opacity-60' : ''}`} style={portalStyle({ color: METIN })}>
      ...
      {tarih && (
        <div data-gorev-tarih className="mt-1.5 text-[11.5px] tabular-nums" style={portalStyle({ color: 'rgba(250,250,249,0.8)' })}>
          {kisaTarih(tarih)}
          {!t.allDay && t.dueTime ? ` ${t.dueTime}` : ''}
          {gecikme ? <span data-gorev-vade={vadeTonu} style={portalStyle({ color: gecikti ? GECIKME_RENK : IKINCIL })}> · {gecikme}</span> : null}
        </div>
      )}
      <div data-gorev-kart-alt className="mt-2.5 flex flex-wrap items-center justify-between gap-1.5 border-t pt-2" style={portalStyle({ borderColor: 'rgba(255,255,255,0.07)' })}>
        <div className="flex flex-wrap items-center gap-1">
          {sutun === 'OPEN' && (
            <button type="button" data-gorev-tasi onClick={() => eylemler.baslat(t.id)} className={tasi} title="Sürüyor sütununa taşı">
              Sürüyor <ArrowRight size={11} />
            </button>
          )}
          {sutun === 'IN_PROGRESS' && (
            <button type="button" data-gorev-tasi onClick={() => eylemler.durumDegistir(t.id, 'OPEN')} className={tasi} title="Açık sütununa geri taşı">
              <ArrowLeft size={11} /> Açık
            </button>
          )}
          {sutun === 'SNOOZED' && (
            <button type="button" data-gorev-tasi onClick={() => eylemler.yenidenAc(t.id)} className={tasi} title="Ertelemeyi kaldır, Açık'a taşı">
              <ArrowLeft size={11} /> Açık
            </button>
          )}
          {sutun === 'DONE' && (
            <button type="button" data-gorev-tasi onClick={() => eylemler.yenidenAc(t.id)} className={tasi} title="Yeniden aç">
              <RotateCcw size={11} /> Yeniden aç
            </button>
          )}
          {sutun !== 'DONE' && (
            <button type="button" data-gorev-tasi data-ton="yesil" onClick={() => eylemler.tamamla(t.id)} className={tasi} title="Bitti sütununa taşı (tamamla)">
              <Check size={11} /> Bitti
            </button>
          )}
        </div>

...
export function Iskelet({ satir = 6 }: { satir?: number }) {
  return (
    <div className="overflow-hidden rounded-xl" style={portalStyle({ border: `1px solid ${KENAR}`, background: 'rgba(255,255,255,0.02)' })} aria-busy="true" aria-label="Yükleniyor">
      <div className="h-8" style={portalStyle({ background: 'rgba(91,141,239,.14)', borderBottom: `1px solid ${KENAR}` })} />
      {Array.from({ length: satir }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-3" style={portalStyle({ borderBottom: i < satir - 1 ? `1px solid ${KENAR}` : undefined })}>
      ...
      <div className="flex items-center justify-center gap-2 py-3 text-[12px]" style={portalStyle({ color: IKINCIL })}>
...
export function HataDurumu({ mesaj, onTekrar, deneniyor }: { mesaj?: string; onTekrar: () => void; deneniyor?: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 rounded-xl px-4 py-10 text-center" style={portalStyle({ border: `1px solid ${KIRMIZI}44`, background: `${KIRMIZI}0d` })} role="alert">
      <span className="flex h-10 w-10 items-center justify-center rounded-full" style={portalStyle({ background: `${KIRMIZI}1a`, color: KIRMIZI })}>
        <WifiOff size={18} />
      </span>
      <div className="text-[13.5px] font-bold" style={portalStyle({ color: METIN })}>
        Veri alınamadı
      </div>
      {mesaj && (
        <div className="max-w-md text-[12px]" style={portalStyle({ color: IKINCIL })}>
          {mesaj}
        </div>
      )}
      <button

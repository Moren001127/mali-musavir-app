...
type MonthlyStatus = { evraklarGeldi:boolean; evraklarIslendi:boolean; beyannameVerildi:boolean; kdvKontrolEdildi:boolean; };
type Taxpayer = { id:string; type:string; firstName?:string; lastName?:string; companyName?:string; taxNumber:string; evrakTeslimGunu?:number; monthlyStatus:MonthlyStatus|null; };
...
export default function MukelleflerPage() {
  ...
  const doneCount = (t:Taxpayer) => [t.monthlyStatus?.evraklarGeldi,t.monthlyStatus?.evraklarIslendi,t.monthlyStatus?.kdvKontrolEdildi,t.monthlyStatus?.beyannameVerildi].filter(Boolean).length;
      ...
      <div className="max-w-[1400px] mx-auto p-6 space-y-5">
        ...
        <div className="rounded-2xl overflow-hidden border border-gray-200/80 shadow-sm">
          ...
          <div className="grid items-center px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-white/70"
            ...
            <span className="text-center">KDV<br/>Kontrol</span>
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0"
                      style={{ background: done
                        ...
                        : 'linear-gradient(135deg,var(--navy),#2a4ea0)' }}>
                      {initials}
                    </div>
                  ...
                  <div className="flex items-center justify-center">
                    {t.evrakTeslimGunu
                      ? <span className="w-6 h-6 rounded-full text-[11px] font-bold flex items-center justify-center"
                          style={{ background:'var(--gold)1a', color:'var(--gold)' }}>{t.evrakTeslimGunu}</span>
                      : <span className="text-gray-200 text-base">—</span>}
                  </div>

                  {/* Checkboxlar */}
                  <div className="flex justify-center"><StatusCheckbox checked={s?.evraklarGeldi??false}   onChange={v=>handleStatus(t.id,'evraklarGeldi',v)}   title="Evraklar Geldi"/></div>
                  <div className="flex justify-center"><StatusCheckbox checked={s?.evraklarIslendi??false} onChange={v=>handleStatus(t.id,'evraklarIslendi',v)} title="Evraklar İşlendi"/></div>
                  <div className="flex justify-center"><StatusCheckbox checked={s?.kdvKontrolEdildi??false} onChange={v=>handleStatus(t.id,'kdvKontrolEdildi',v)} title="KDV Kontrol"/></div>
                  <div className="flex justify-center"><StatusCheckbox checked={s?.beyannameVerildi??false} onChange={v=>handleStatus(t.id,'beyannameVerildi',v)} title="Beyanname Verildi"/></div>
                    ...
                    <div className="flex gap-[3px]">
                      {[0,1,2,3].map(n=>(
                        <div key={n} className="w-3.5 h-1 rounded-full transition-all duration-300"
                          style={{ background: n<cnt ? (done?'#16a34a':'var(--gold)') : '#e5e7eb' }}/>
                      ))}
                    </div>
          ...
          {!isLoading && taxpayers.length > 0 && (
            <div className="px-4 py-2.5 border-t border-gray-100 flex items-center justify-between bg-gray-50/80">
              <span className="text-[11px] text-gray-400">{taxpayers.length} kayıt</span>

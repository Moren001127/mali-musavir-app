...
export function PortalTabState({ children }: { children: React.ReactNode }) {
  return <div className="flex min-h-[160px] items-center justify-center text-[13px]" style={portalStyle({ color: MUTED })}>{children}</div>;
}
...
export function BosDurum({ icon: Icon, title, text, renk = MUTED, children }: { icon: React.ElementType; title: string; text: string; renk?: string; children?: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <span className="mb-3 flex h-10 w-10 items-center justify-center" style={portalStyle(ikonRozeti(renk))}>
        <Icon size={18} />
      </span>
      <p className="text-[14px] font-bold" style={portalStyle({ color: TEXT })}>{title}</p>
      <p className="mt-1 max-w-md text-[13px]" style={portalStyle({ color: MUTED })}>{text}</p>
      {children}
    </div>
  );
}
...
export function Th({ children, right, center, style }: { children?: React.ReactNode; right?: boolean; center?: boolean; style?: React.CSSProperties }) {
  return <th style={portalStyle({ ...HUCRE_BASLIK, textAlign: right ? 'right' : center ? 'center' : 'left', ...style })}>{children}</th>;
}
...
export function Td({ children, right, center, muted, tabular, className = '', style, colSpan }: { children?: React.ReactNode; right?: boolean; center?: boolean; muted?: boolean; tabular?: boolean; className?: string; style?: React.CSSProperties; colSpan?: number }) {
  return (
    <td colSpan={colSpan} className={`${tabular ? 'tabular-nums' : ''} ${className}`} style={portalStyle({ ...HUCRE, textAlign: right ? 'right' : center ? 'center' : 'left', color: muted ? MUTED : TEXT, ...style })}>
      {children}
    </td>
  );
}
...
export function DocBtn({ label, disabled, busy, onClick, muted }: { label: string; disabled?: boolean; busy?: boolean; onClick: () => void; muted?: boolean }) {
  return (
    <button
      ...
      style={portalStyle({ color: muted ? FAINT : MUTED })}
    >
...
export function Cip({ renk: _renk, children, className = '' }: { renk?: string; children: React.ReactNode; className?: string }) {
  return (
    <span className={`whitespace-nowrap text-[13px] font-medium ${className}`} style={portalStyle({ color: 'rgba(250,250,249,0.80)' })}>
      {children}
    </span>
  );
}
...
export function Dipnot({ children }: { children: React.ReactNode }) {
  return <p className="mt-2.5 text-[11.5px]" style={portalStyle({ color: FAINT })}>{children}</p>;
}

/** Bağlantı görünümlü düğme (nötr, 36px). */
export function BaglantiDugme({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex h-9 items-center gap-1.5 px-3.5 text-[13px] font-medium transition hover:brightness-125" style={portalStyle(NOTR_DUGME)}>
      {children}
    </Link>
  );
}
...
export function PlaceholderTab({
  ...
  return (
    <BosDurum icon={Icon} title={title} text={description}>
      {comingSoon && (
        <span className="mt-3 rounded-md px-2 py-0.5 text-[11.5px] font-medium" style={portalStyle({ border: `1px solid ${LINE}`, color: MUTED })}>Yakında</span>
      )}
      {linkLabel && linkHref && (
        <div className="mt-4">

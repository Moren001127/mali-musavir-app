...
export function AccordionRow({
  ...
  return (
    ...
    >
      <span className="absolute inset-y-0 left-0 w-[3px]" style={{ background: renk, opacity: open ? 1 : 0.55 }} />
      <button
        type="button"
        aria-expanded={open}
        onClick={onToggle}
      ...
      >
        <span className="flex h-[34px] w-[34px] shrink-0 items-center justify-center" style={ikonRozeti(renk)}>
          <Icon size={16} />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block truncate text-[14px] font-bold leading-5" style={{ color: TEXT }}>{title}</span>
          <span className="mt-[2px] block truncate text-[11.5px]" style={{ color: MUTED }}>{subtitle}</span>
        </span>
        {filled && (
          <span className="hidden items-center gap-1.5 text-[11.5px] font-medium sm:inline-flex" style={{ color: GREEN }}>
        ...
        <ChevronDown
          ...
          style={{ color: open ? renk : MUTED, transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}
        />
      </button>
      {open && (
        <div className="border-t px-4 py-4 pl-[19px] sm:px-5 sm:pl-[23px]" style={{ borderColor: HAIR }}>
          {children}
        </div>
      )}
    </section>

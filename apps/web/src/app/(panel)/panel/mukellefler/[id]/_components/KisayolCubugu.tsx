  ...
  return (
    <section className="overflow-hidden" style={kartZemin()}>
      ...
      <div className="flex flex-wrap items-center gap-2 px-3 py-2">
        ...
        >
          <span className="flex h-[34px] w-[34px] shrink-0 items-center justify-center" style={ikonRozeti(GOLD)}>
            <Zap size={16} />
          </span>
          <span className="min-w-0">
            <span className="block truncate text-[14px] font-bold leading-5" style={{ color: TEXT }}>Kısayollar &amp; Sorgulamalar</span>
            <span className="block truncate text-[11.5px]" style={{ color: MUTED }}>{vkn ? `VKN ${vkn} · ${toplam} bağlantı` : `${toplam} bağlantı`}</span>
          </span>
        </button>

        <span className="mx-1 hidden h-5 w-px sm:block" style={{ background: LINE }} />
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {KISAYOL_GRUPLARI.map((g) => (
            <button
              ...
              style={{ border: `1px solid ${LINE}`, color: MUTED, background: 'rgba(255,255,255,0.02)' }}
              title={`${g.baslik} — ${g.aciklama}`}
            >
              <span className="h-1.5 w-1.5 rounded-full" style={{ background: g.renk, opacity: 0.9 }} />
              {g.baslik}
              <span className="tabular-nums" style={{ color: FAINT }}>{g.items.length}</span>
            </button>
          ))}
        </div>
        ...
        <div className="relative w-full shrink-0 sm:w-[170px]">
          <Search size={13} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: MUTED }} />
          <input
            value={q}
            ...
            style={{ background: 'rgba(0,0,0,0.30)', color: TEXT }}
          />
        </div>
        ...
        >
          <ChevronDown size={16} className="transition-transform duration-200" style={{ transform: gosterilenAcik ? 'rotate(180deg)' : 'rotate(0deg)' }} />
        </button>
      </div>

      {/* Açılan sıkı ızgara */}
      {gosterilenAcik && (
        <div className="space-y-4 border-t px-3 pb-3 pt-3" style={{ borderColor: HAIR }}>
          {gruplar.map((g) => {
            const Icon = g.ikon;
            return (
              <div key={g.id} ref={(el) => { grupRef.current[g.id] = el; }} className="scroll-mt-2">
                <div className="mb-2 flex items-center gap-2 border-b pb-1.5" style={{ borderColor: HAIR }}>
                  <Icon size={13} style={{ color: g.renk }} />
                  <span className="text-[12px] font-bold" style={{ color: TEXT }}>{g.baslik}</span>
                  <span className="text-[11.5px]" style={{ color: FAINT }}>· {g.aciklama}</span>
                  <span className="ml-auto text-[11.5px] tabular-nums" style={{ color: FAINT }}>{g.items.length}</span>
                </div>
...
export function KisayolLogo({ k }: { k: Kisayol }) {
  ...
  return (
    <span
      ...
      style={{ background: `${k.renk}22`, color: k.renk, border: `1px solid ${k.renk}55` }}
    >
      {k.kisaltma}
    </span>
  );
...
export function KisayolCard({ k, onClick }: { k: Kisayol; onClick: () => void }) {
  return (
    <button
      ...
      style={{ border: `1px solid ${LINE}`, background: 'rgba(255,255,255,0.02)', borderRadius: R_ALAN }}
    >
      <KisayolLogo k={k} />
      <span className="min-w-0 flex-1 truncate text-[13px] font-medium" style={{ color: TEXT }}>
        {k.label}
      </span>
      <ExternalLink size={12} className="shrink-0" style={{ color: FAINT }} />
    </button>
  );
}

  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-6"
      ...
      <div
        ...
        style={{ border: `1px solid ${LINE}`, borderRadius: R_KART, background: CARD, transform: shown ? 'scale(1)' : 'scale(0.94)', opacity: shown ? 1 : 0, transition: 'transform .18s ease, opacity .18s ease' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center gap-3 border-b px-4 py-3" style={{ borderColor: HAIR }}>
          <div className="min-w-0 flex-1 truncate text-[14px] font-bold" style={{ color: TEXT }}>{title}</div>
          ...
          <button type="button" onClick={onClose}
            ...
            style={NOTR_DUGME} title="Kapat (Esc)">
            <X size={16} />
          </button>
        </div>

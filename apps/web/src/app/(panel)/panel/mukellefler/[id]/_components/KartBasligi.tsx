...
export function KartBasligi({
  ...
  return (
    <header className="overflow-hidden" style={kartZemin()}>
      <div className="flex flex-wrap items-center gap-3 px-4 py-3">
        ...
        {/* Avatar + doluluk */}
        <div className="relative flex h-12 w-12 shrink-0 items-center justify-center" style={{ border: `1px solid ${LINE}`, background: 'rgba(0,0,0,0.30)', borderRadius: R_ALAN }}>
          {logoUrl ? (
            <div className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url(${logoUrl})`, borderRadius: R_ALAN }} />
          ) : (
            <span className="text-[18px] font-bold" style={{ color: TEXT }}>{avatarText}</span>
          )}
          {compScore != null && (
            <span
              ...
              style={{ background: CARD, border: `1px solid ${LINE}`, color: compColor }}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <h1 className="min-w-0 truncate text-[18px] font-bold leading-6 tracking-[-0.01em]" style={{ color: TEXT }}>{currentName}</h1>
            {!isNew && (
              <div className="relative">
                <button
                  ...
                  style={{ border: `1px solid ${durumRenk}55`, background: `${durumRenk}14`, color: durumRenk }}
                  title="Durumu değiştir"
                >
                  <span className="h-1.5 w-1.5 rounded-full" style={{ background: durumRenk }} />
                  {isTaxpayerActive ? 'Aktif' : 'Pasif'}
                  <ChevronDown size={11} style={{ opacity: 0.7 }} />
                </button>
                {activeActionOpen && (
                  <div className="absolute left-0 top-[calc(100%+6px)] z-30 w-[150px] p-1" style={{ background: '#14110e', border: `1px solid ${LINE}`, borderRadius: R_ALAN, boxShadow: '0 18px 48px rgba(0,0,0,0.6)' }}>
                    <button
                      type="button"
                      className="w-full rounded-[6px] px-3 py-2 text-left text-[13px] font-medium transition hover:bg-white/[0.06]"
                      style={{ color: isTaxpayerActive ? RED : GREEN }}
                      onClick={() => {
            ...
            <span className="inline-flex h-6 items-center rounded-md px-2 text-[11.5px] font-medium" style={{ border: `1px solid ${LINE}`, color: MUTED }}>
      ...
      {!isNew && eksikler.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 px-4 py-1.5 text-[11.5px]" style={{ background: 'rgba(240,183,85,0.08)', borderTop: '1px solid rgba(240,183,85,0.22)', color: AMBER }}>
          <AlertTriangle size={12} />
          <span className="font-medium">Eksik:</span>
          <span style={{ color: 'rgba(240,183,85,0.85)' }}>
...
export function CardNavButtons({ cardNav, isNew, router }: { cardNav: any; isNew: boolean; router: any }) {
  ...
  return (
    <div className="inline-flex h-9 overflow-hidden" style={{ border: `1px solid ${LINE}`, borderRadius: R_ALAN, background: 'rgba(255,255,255,0.03)' }}>
      <button
        type="button"
      ...
      <span className="inline-flex items-center border-x px-2.5 text-[11.5px] font-bold tabular-nums" style={{ borderColor: LINE, color: TEXT }}>

...
/*
 * 2026-09-25 (2. tur) EKRAN = MÜKELLEF SEÇİMLİ (Muzaffer Bey, panoyu canlıda görüp reddetti:
 * "bu şekilde pano olmasın, tüm mükellefler seçilemesin, mükellef seçilsin ve bütün genel sorgulama
 * tablosu açılsın"). Mükellef seçili DEĞİLKEN hiç tablo çizilmez, tek bir yönlendirme kartı durur;
 * mükellef seçilince o mükellefin 5 tablosu alt alta açılır (tür sekmesiyle teke indirilebilir).
 * Mükellef seçici artık "Tüm mükellefler" seçeneği sunmaz. Mükellef panosu (1. tur) KALDIRILDI —
 * API'deki /genel-sorgular/pano ucu da geri alındı.
 *
 * 2026-09-25 (1. tur) taşma düzeltmeleri KALDI: sayfa 1280 → 1560px; tablolardan "Son sorgu" sütunu
 * kalktı (kart başlığında zaten yazıyor); sütun genişlikleri gerçek en-az toplamına çekildi; mükellef
 * hep seçili olduğu için Mükellef / VKN sütunları da kalktı (bkz. GuncelTablo EN_AZ_GENISLIK).
 *
...
function GenelSorgularIcerik() {
  ...
  return (
    <div className="gs mx-auto w-full max-w-[1560px]" data-gs-kok>
      ...
      {!suzgec.mukellefId ? (
        /* Mükellef seçilmeden tablo yok: sorgular mükellef başına çalışır, karışık liste olmaz. */
        <div className="gs-secim-bekliyor">
          <span className="gs-secim-simge" aria-hidden><Users size={26} strokeWidth={1.8} /></span>
          <b>Bir mükellef seçin</b>
          <span>
            Yukarıdaki <i>Mükellef</i> kutusundan birini seçtiğinizde o mükellefin Dijital Vergi Dairesi durumu —
            vergi borcu, e-haciz, yoklama / denetim, POS ve gelen e-arşiv — burada tablolar hâlinde açılır.
          </span>
        </div>
      ) : (
        <>
          <TurSekmeleri
            ...
            suzgec={{ taxpayerId: suzgec.mukellefId, donem: suzgec.donem || undefined }}
          />

  ...
  hucre: (s) => {
    ...
    return (
      <div className="flex min-w-0 items-center gap-2">
        ...
        >
          <Building2 size={12} style={{ color: IKINCIL, flexShrink: 0 }} />
          <span className="truncate">{ad}</span>
        </Link>
        {vkn && <span className="hidden text-[10.5px] tabular-nums sm:inline" style={{ color: SONUK, fontFamily: 'JetBrains Mono, monospace' }}>{vkn}</span>}
      </div>
    );
  },
};
const DONEM: Sutun = { baslik: 'Dönem', genislik: 120, hucre: (s) => <span className="text-[12.5px]" style={{ color: IKINCIL }}>{donemEtiketi(s.donem) || '—'}</span> };
const SORGU_TARIHI: Sutun = { baslik: 'Sorgu tarihi', genislik: 140, hucre: (s) => <span className="text-[12.5px] tabular-nums" style={{ color: IKINCIL }}>{tarihSaat(s.sorguTarihi) || '—'}</span> };
const OZET: Sutun = { baslik: 'Özet', hucre: (s) => <span className="block truncate text-[12.5px]" style={{ color: 'rgba(250,250,249,0.8)' }} title={s.ozet || ''}>{s.ozet || '—'}</span> };
const tutarSutunu = (baslik: string): Sutun => ({
  ...
  hucre: (s) => {
    ...
    return <span className="text-[13px] font-medium tabular-nums" style={{ color: n === null ? SONUK : METIN }}>{para(n)}</span>;
  },
});
  ...
  E_HACIZ: [
    ...
    OZET,
    {
      ...
      hucre: (s) => {
        ...
        return d ? <span className="inline-flex whitespace-nowrap rounded-md px-1.5 py-[2px] text-[10.5px] font-medium leading-4" style={CIP_NOTR}>{degerMetni(d)}</span> : <span style={{ color: SONUK }}>—</span>;
      },
    },
  ],
  YOKLAMA_DENETIM: [
    MUKELLEF,
    { baslik: 'Tutanak tarihi', genislik: 130, hucre: (s) => <span className="text-[12.5px] tabular-nums" style={{ color: IKINCIL }}>{tarihKisa(typeof s.veri?.tarih === 'string' ? s.veri.tarih : null) || '—'}</span> },
    SORGU_TARIHI,
    OZET,
  ],
  ...
  GELEN_EARSIV: [
    ...
    SORGU_TARIHI,
    {
      ...
      hucre: (s) => {
        ...
        return <span className="text-[12.5px] tabular-nums" style={{ color: n === null ? SONUK : METIN }}>{n === null ? '—' : n.toLocaleString('tr-TR')}</span>;
      },
    },
    tutarSutunu('Toplam'),
  ],
...
export function SonucGrubu(p: SonucGrubuProps) {
  ...
  return (
    <section className="mb-4 overflow-hidden rounded-xl" style={{ border: `1px solid ${KENAR_NOTR}`, background: 'rgba(255,255,255,0.02)' }}>
      {/* Grup bandı */}
      <div className="flex flex-wrap items-center gap-2.5 px-3 py-2.5" style={{ background: GRUP_ZEMIN, borderBottom: GRUP_CIZGI, borderLeft: `4px solid ${renk}` }}>
        ...
        {p.yukleniyor && <span className="text-[11px]" style={{ color: SONUK }}>· yükleniyor…</span>}
        {earsiv && (
          <div className="ml-auto inline-flex items-center rounded-full p-[3px]" style={{ background: 'rgba(0,0,0,0.32)', border: '1px solid rgba(255,255,255,0.08)' }} role="tablist">
            ...
            <table className="w-full" style={{ borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 720 }}>
              ...
              <thead>
                <tr style={{ background: 'rgba(212,184,118,0.07)' }}>
                  <th style={{ ...HUCRE_BASLIK, padding: '7px 4px' }}><span className="sr-only">Detay</span></th>
                  {sutunlar.map((s) => (
                    <th key={s.baslik} style={{ ...HUCRE_BASLIK, textAlign: s.sag ? 'right' : 'left' }}>{s.baslik}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <Fragment key={s.id}>
                      <tr
                        ...
                        style={{ background: acikMi ? 'rgba(212,184,118,0.05)' : 'transparent', boxShadow: acikMi ? `inset 3px 0 0 ${GOLD}` : undefined }}
                        title={acikMi ? 'Detayı kapat' : 'Detayı aç'}
                      >
                        <td style={{ ...HUCRE, padding: '8px 4px', textAlign: 'center', color: IKINCIL }}>
                          {acikMi ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                        </td>
                        {sutunlar.map((c) => (
                          <td key={c.baslik} style={{ ...HUCRE, minWidth: 0, textAlign: c.sag ? 'right' : 'left' }}>{c.hucre(s)}</td>
                        ))}
                      </tr>
                      {acikMi && (
                        <tr>
                          <td colSpan={SUTUN} style={{ ...HUCRE, padding: 0, background: 'rgba(0,0,0,0.18)' }}>
                            <Detay sonuc={s} />
                          </td>
                        </tr>
                      )}
                ...
                {p.rows.length === 0 && p.yukleniyor && (
                  <tr><td colSpan={SUTUN} style={{ ...HUCRE, textAlign: 'center', color: SONUK, fontSize: 12 }}>Yükleniyor…</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function Detay({ sonuc }: { sonuc: SorguSonucu }) {
  ...
  return (
    <div className="px-4 py-3" style={{ borderLeft: `3px solid ${GOLD}55` }}>
      <div className="mb-2 flex flex-wrap items-center gap-1.5">
        {sonuc.whatsappGonderildiMi ? (
          <span className="inline-flex items-center gap-1 rounded-md px-1.5 py-[2px] text-[10.5px] font-medium leading-4" style={{ background: 'rgba(37,211,102,0.10)', border: '1px solid rgba(37,211,102,0.35)', color: '#5fd38a' }} title="Sonuç mükellefe WhatsApp ile iletildi">
            <MessageCircle size={10} /> WhatsApp gönderildi
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 rounded-md px-1.5 py-[2px] text-[10.5px] font-medium leading-4" style={CIP_NOTR} title="Mükellefe WhatsApp gönderilmedi">
            <MessageCircle size={10} /> WhatsApp gönderilmedi
          </span>
        )}
        {sonuc.kaynak && <span className="inline-flex rounded-md px-1.5 py-[2px] text-[10.5px] font-medium leading-4" style={CIP_NOTR} title="Kaynak">{sonuc.kaynak}</span>}
        {sonuc.ozet && <span className="text-[12px]" style={{ color: IKINCIL }}>· {sonuc.ozet}</span>}
      </div>
      {girdiler.length === 0 ? (
        <div className="text-[12px]" style={{ color: SONUK }}>Bu sorgu için ayrıntı verisi yok.</div>
      ) : (
...
function AnahtarDeger({ girdiler, seviye = 0 }: { girdiler: Array<[string, unknown]>; seviye?: number }) {
  return (
    <dl className="grid gap-x-4 gap-y-1.5" style={{ gridTemplateColumns: 'minmax(120px, 200px) 1fr' }}>
      {girdiler.map(([k, v]) => {
        ...
        if (nesne && seviye < 3) {
          ...
          return (
            <Fragment key={k}>
              <dt style={AD_STIL} className="pt-0.5">{anahtarAdi(k)}{Array.isArray(v) ? ` (${v.length})` : ''}</dt>
              <dd className="min-w-0">
                {ic.length === 0 ? (
                  <span className="text-[12.5px]" style={{ color: SONUK }}>—</span>
                ) : hepsiBasit && Array.isArray(v) ? (
                  <span className="text-[12.5px]" style={{ color: 'rgba(250,250,249,0.85)' }}>{ic.map(([, x]) => degerMetni(x)).join(', ')}</span>
                ) : (
                  <div className="rounded-lg px-3 py-2" style={{ border: `1px solid ${KENAR_NOTR}`, background: 'rgba(255,255,255,0.02)' }}>
        ...
        return (
          <Fragment key={k}>
            <dt style={AD_STIL} className="pt-0.5">{anahtarAdi(k)}</dt>
            <dd className="min-w-0 break-words text-[12.5px]" style={{ color: 'rgba(250,250,249,0.85)' }}>{degerMetni(v)}</dd>
          </Fragment>
        );
      })}
    </dl>

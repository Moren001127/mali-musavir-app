      ...
      {SORGU_TURLERI.map((t) => {
        ...
        return (
          <button
            ...
            style={{ ...kartZemini(renk), ...(secili ? { border: `1px solid ${renk}66` } : {}) }}
          >
            <div className="absolute left-0 top-0 h-full w-[3px]" style={{ background: `linear-gradient(180deg, ${renk}, ${renk}44)` }} />
            <div className="text-[10.5px] font-semibold uppercase tracking-[.08em]" style={{ color: IKINCIL }}>
              {SORGU_TURU_ADI[t]}
            </div>
            <div className="mt-1 flex items-baseline gap-1.5">
              <span className="text-[20px] font-semibold leading-none tabular-nums" style={{ color: METIN }}>
                {yukleniyor && !k ? '…' : (k?.adet ?? 0).toLocaleString('tr-TR')}
              </span>
              <span className="text-[11px]" style={{ color: SONUK }}>sorgu</span>
            </div>
            <div className="mt-1 truncate text-[11px]" style={{ color: SONUK }}>

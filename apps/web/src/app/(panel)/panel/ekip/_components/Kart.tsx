...
>(function Kart({ renk = TEMA.mavi, baslik, aciklama, sag, dolguYok = false, serit = false, secili = false, className = '', style, children, id }, ref) {
  return (
    <section
      ref={ref}
      id={id}
      className={`relative min-w-0 overflow-hidden rounded-2xl ${className}`}
      style={{ background: TEMA.kartZemin, border: `1px solid ${secili ? `${renk}66` : TEMA.kartKenar}`, boxShadow: '0 18px 44px rgba(0,0,0,0.24)', ...style }}
    >
      {serit ? (
      ...
      <div className="pointer-events-none absolute -right-10 -top-12 h-32 w-32 rounded-full opacity-[0.10]" style={{ background: `radial-gradient(circle, ${renk}, transparent 68%)` }} />
      {baslik && (
...
export function KPI({ etiket, deger, altBilgi, renk = TEMA.mavi, ikon, vurgu = false, onClick, title }: { etiket: string; deger: string | number; altBilgi?: string; renk?: string; ikon?: ReactNode; vurgu?: boolean; onClick?: () => void; title?: string }) {
  ...
  const stil: CSSProperties = {
    background: vurgu ? `linear-gradient(140deg, ${renk}1f, rgba(255,255,255,0.01) 60%)` : TEMA.kartZemin,
    border: `1px solid ${vurgu ? `${renk}3d` : TEMA.kartKenar}`,
...
export function Avatar({ kisaltma, boyut = 32, durum = 'bos', title }: { kisaltma: string; boyut?: 24 | 28 | 32 | 40; durum?: 'bos' | 'calisiyor' | 'hata' | 'siz'; title?: string }) {
  ...
  return (
    <span
      ...
      style={{
        background: durum === 'bos' ? 'rgba(255,255,255,0.05)' : `${renk}1a`,

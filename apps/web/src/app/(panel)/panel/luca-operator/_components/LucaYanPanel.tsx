    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        <Bolum ikon={<MapIcon size={13} />} baslik="Öğrendiği Menüler" sayi={haritalar.length}>
          {haritalar.length === 0 ? (
            <p className="text-[11px]" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>
            ...
            <div className="flex flex-col gap-1.5">
              {haritalar.map((h) => (
                ...
                >
                  <div className="truncate text-[11px]" style={portalStyle({ color: 'rgba(250,250,249,0.8)' })}>
                    {h.baslik}
                  </div>
                  <div className="text-[10px]" style={portalStyle({ color: ACCENT })}>
        ...
        <Bolum ikon={<Scale size={13} />} baslik="Ofis Kuralları" sayi={kurallar.length} varsayilanAcik>
          {kurallar.length === 0 ? (
            <p className="text-[11px] leading-relaxed" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>
              ...
              {kurallar.map((k) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="text-[11px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>
                      {k.baslik}
                    </div>
                    <div className="text-[11px] leading-snug" style={portalStyle({ color: 'rgba(250,250,249,0.55)' })}>
                      {k.kural}
                    </div>
                  </div>
                  <button
                    ...
                    title="Kuralı sil"
                    style={portalStyle({ color: 'rgba(248,113,113,0.8)' })}
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
          ...
          {beceriler.length === 0 ? (
            ...
            <div className="flex flex-col gap-1.5">
              {beceriler.map((b) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[11px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>
                      {b.ad}
                    </div>
                    <div className="text-[10px]" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>
                      {b.adimSayisi} adım
                    </div>
                  </div>
                  <button
        ...
        <Bolum ikon={<Info size={13} />} baslik="Nasıl Çalışır">
          <ul className="flex flex-col gap-1.5 text-[11px] leading-snug" style={portalStyle({ color: 'rgba(250,250,249,0.6)' })}>
            ...
            <li>
              <b style={portalStyle({ color: 'rgba(250,250,249,0.85)' })}>Gönder / Kaydet / Onayla / Tahakkuk</b> adımında durur,

    ...
    <div className="mx-auto max-w-4xl space-y-5 pb-12">
      <header className="relative overflow-hidden rounded-2xl border p-5" style={portalStyle({ borderColor: 'rgba(255,255,255,0.08)', background: 'radial-gradient(120% 140% at 0% 0%, rgba(212,184,118,0.16), transparent 45%), #0f0d0b' })}>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-[24px] font-semibold leading-tight" style={portalStyle({ color: '#fafaf9' })}>Mükellef Portal Erişimi</h1>
            <p className="text-[12.5px]" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>Mükellefe self-servis portal girişi açın, e-posta ve şifre belirleyin.</p>
          </div>
        </div>
      </header>
      ...
      {isLoading ? (
        <div className="py-12 text-center"><Loader2 size={22} className="animate-spin inline" style={portalStyle({ color: GOLD })} /></div>
      ) : taxpayers.length === 0 ? (
        <p className="py-12 text-center text-[13px]" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>Mükellef bulunamadı.</p>
      ) : (
...
function ErisimSatiri({ taxpayer, open, onToggle }: { taxpayer: Taxpayer; open: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' })}>
      ...
      {open && (
        <div className="border-t px-4 pb-4 pt-3 space-y-3" style={portalStyle({ borderColor: 'rgba(255,255,255,0.06)' })}>
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                <input value={portalEmail} onChange={(e) => setPortalEmail(e.target.value)}
                  ...
                  style={portalStyle({ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fafaf9' })} />
                <div className="relative">
                  <KeyRound size={14} className="absolute left-3 top-1/2 -translate-y-1/2" style={portalStyle({ color: 'rgba(250,250,249,0.35)' })} />
              ...
              <p className="text-[11.5px]" style={portalStyle({ color: 'rgba(250,250,249,0.35)' })}>

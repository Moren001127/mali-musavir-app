    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div
          ...
          style={portalStyle({ background: 'linear-gradient(90deg, #f43f5e, #fb7185, #fbbf24, #d4b876)' })}
        />
        ...
        <div className="mt-2 flex items-center justify-between gap-3 flex-wrap">
          <h1 className="flex items-center gap-2.5 text-[28px] font-semibold leading-tight" style={portalStyle({ color: '#fafaf9' })}>
            <span
              ...
              style={portalStyle({ background: 'linear-gradient(135deg, #f43f5e, #be123c)', boxShadow: '0 6px 18px rgba(244,63,94,0.35)' })}
            >
              <Lock size={22} style={portalStyle({ color: '#ffffff' })} />
            </span>
            Kilitli Modüller
          </h1>
          <button
            ...
            style={portalStyle({ background: 'linear-gradient(135deg, #f43f5e, #be123c)', color: '#fff' })}
          >
            <Plus size={16} />
            Yeni Modül Kilitle
          </button>
      ...
      >
        ...
        >
          <h2 className="text-[14px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>
            Kilitli Dosyalar ({list.length})
          </h2>
          <span className="text-xs" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>
            Otomatik yenileme: 30sn
          </span>
        </div>

        {isLoading && (
          <div className="px-5 py-12 text-center text-sm" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>
            Yükleniyor...
          </div>
        )}
        ...
        {!isLoading && list.length === 0 && (
          <div className="px-5 py-12 text-center">
            <ShieldCheck size={48} style={portalStyle({ color: ROSE, margin: '0 auto 12px' })} />
            <h3 className="text-[14px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>
              Henüz kilitli modül yok
            </h3>
            <p className="text-xs mt-1" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>
      ...
      >
        <p className="font-semibold mb-2" style={portalStyle({ color: ROSE })}>Nasıl çalışır?</p>
        ...
        <p className="mt-3 text-xs" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <span className="text-[10px] uppercase font-bold tracking-[.16em]" style={portalStyle({ color })}>
          {label}
        </span>
      ...
      <p className="mt-3 text-[34px] font-bold leading-none" style={portalStyle({ color: value > 0 ? color : '#fafaf9' })}>
        {value}
      </p>
      <p className="text-[11px] mt-1.5" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>
        {description}
      </p>
    </div>
  );
    ...
    <div className="px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-mono font-semibold break-all" style={portalStyle({ color: '#fafaf9' })}>
            {m.path}
          </p>
          <p className="text-[12px] mt-1" style={portalStyle({ color: 'rgba(250,250,249,0.7)' })}>
            {m.reason}
          </p>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-[11px]" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>
          ...
          {m.notes && (
            <p className="text-[11px] mt-2 italic" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>
              Not: {m.notes}
            </p>
          )}
        </div>
...
function AddModal({
  ...
  return (
    <div
      ...
      style={portalStyle({ background: 'rgba(0,0,0,0.6)' })}
      onClick={onClose}
    >
      <div
      ...
      >
        <div
          ...
          style={portalStyle({ borderBottom: '1px solid rgba(255,255,255,0.05)' })}
        >
          <h3 className="text-[15px] font-semibold flex items-center gap-2" style={portalStyle({ color: '#fafaf9' })}>
            <span
              className="grid h-7 w-7 place-items-center rounded-lg"
              style={portalStyle({ background: 'linear-gradient(135deg, #f43f5e, #be123c)', boxShadow: '0 4px 12px rgba(244,63,94,0.35)' })}
            >
              <Lock size={14} style={portalStyle({ color: '#ffffff' })} />
            </span>
            Yeni Modül Kilitle
          </h3>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg flex items-center justify-center hover:opacity-80"
            style={portalStyle({ background: 'rgba(255,255,255,0.03)' })}
          >
            <X size={14} style={portalStyle({ color: 'rgba(250,250,249,0.7)' })} />
          </button>
        </div>

        <div className="px-5 py-5 space-y-4">
          <div>
            <label className="block text-xs font-medium mb-1.5" style={portalStyle({ color: 'rgba(250,250,249,0.7)' })}>
              Dosya Yolu <span style={portalStyle({ color: '#f43f5e' })}>*</span>
            </label>
            <input
              type="text"
              value={path}
            ...
            />
            <p className="text-[11px] mt-1" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={portalStyle({ color: 'rgba(250,250,249,0.7)' })}>
              Kilitleme Sebebi <span style={portalStyle({ color: '#f43f5e' })}>*</span>
            </label>
            <input
              type="text"
              value={reason}
        ...
        <div
          ...
          style={portalStyle({ borderTop: '1px solid rgba(255,255,255,0.05)' })}
        >
          <button
            onClick={onClose}

...
export function GonderimOnayKutusu({
  ...
  return createPortal((
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={portalStyle({ background: 'rgba(0,0,0,0.66)' })} onClick={() => { if (!gonderiliyor) onKapat(); }}>
      ...
      >
        <div className="absolute inset-x-0 top-0 h-[2px]" style={portalStyle({ background: 'linear-gradient(90deg, transparent, #d4b876, transparent)', opacity: 0.8 })} />
        <div className="flex items-start gap-3 px-5 pt-5">
          <div className="grid flex-none place-items-center" style={portalStyle({ width: 40, height: 40, borderRadius: 11, background: channel === 'EMAIL' ? 'rgba(56,189,248,0.14)' : 'rgba(34,197,94,0.14)', border: `1px solid ${channel === 'EMAIL' ? 'rgba(56,189,248,0.3)' : 'rgba(34,197,94,0.3)'}` })}>
          ...
          <button type="button" onClick={onKapat} disabled={gonderiliyor} title="Kapat" className="inline-flex h-8 w-8 flex-none items-center justify-center rounded-[8px]" style={portalStyle({ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(250,250,249,0.6)' })}>
            <IconX size={15} />
          </button>
        </div>

        {n > 0 && (
          <div className="mx-5 mt-4 rounded-[10px] px-3 py-2.5" style={portalStyle({ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' })}>
        ...
        {testMode === true && (
          <div className="mx-5 mt-3 flex items-start gap-2 rounded-[10px] px-3 py-2.5 text-[12.5px]" style={portalStyle({ background: 'rgba(251,191,36,0.10)', border: '1px solid rgba(251,191,36,0.35)', color: '#fde68a' })}>
        ...
        <div className="mt-5 flex items-center justify-end gap-2 px-5 pb-5">
          <button type="button" onClick={onKapat} disabled={gonderiliyor} className="inline-flex h-9 items-center rounded-[9px] px-3.5 text-[12.5px] font-semibold disabled:opacity-50" style={portalStyle({ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(250,250,249,0.72)' })}>
            Vazgeç
          </button>
          <button
            type="button"

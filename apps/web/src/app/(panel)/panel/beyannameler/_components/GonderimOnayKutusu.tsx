...
export function GonderimOnayKutusu({
  ...
  return createPortal((
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.66)' }} onClick={() => { if (!gonderiliyor) onKapat(); }}>
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`${kanal} ile gönderim onayı`}
        className="relative w-full max-w-[520px] overflow-hidden rounded-[14px]"
        style={{ background: 'linear-gradient(160deg, #17140f, #100e0b)', border: '1px solid rgba(212,184,118,0.28)', boxShadow: '0 28px 70px -30px rgba(0,0,0,0.9)' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="absolute inset-x-0 top-0 h-[2px]" style={{ background: 'linear-gradient(90deg, transparent, #d4b876, transparent)', opacity: 0.8 }} />
        <div className="flex items-start gap-3 px-5 pt-5">
          <div className="grid flex-none place-items-center" style={{ width: 40, height: 40, borderRadius: 11, background: channel === 'EMAIL' ? 'rgba(56,189,248,0.14)' : 'rgba(34,197,94,0.14)', border: `1px solid ${channel === 'EMAIL' ? 'rgba(56,189,248,0.3)' : 'rgba(34,197,94,0.3)'}` }}>
            <Ikon size={19} style={{ color: channel === 'EMAIL' ? '#7dd3fc' : '#86efac' }} />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[12.5px] leading-relaxed" style={{ color: 'rgba(250,250,249,0.55)' }}>
              {n > 0
          ...
          <button type="button" onClick={onKapat} disabled={gonderiliyor} title="Kapat" className="inline-flex h-8 w-8 flex-none items-center justify-center rounded-[8px]" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(250,250,249,0.6)' }}>
            <IconX size={15} />
          </button>
        </div>

        {n > 0 && (
          <div className="mx-5 mt-4 rounded-[10px] px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <ul className="space-y-1 text-[12.5px]">
              {gosterilen.map((mk) => (
                <li key={mk.ad} className="flex items-center justify-between gap-3">
                  <span className="truncate" style={{ color: '#fafaf9' }}>{mk.ad}</span>
                  <span className="shrink-0 tabular-nums" style={{ color: 'rgba(250,250,249,0.5)' }}>{mk.sayi} kayıt</span>
                </li>
              ))}
              {kalan > 0 && <li className="text-[12px]" style={{ color: 'rgba(250,250,249,0.45)' }}>+ {kalan} mükellef daha</li>}
            </ul>
          </div>
        )}

        {ozet.engeller.length > 0 && (
          <div className="mx-5 mt-3 text-[12px]" style={{ color: 'rgba(250,250,249,0.5)' }}>
            Atlanacak: {ozet.engeller.map(([neden, sayi]) => `${sayi} kayıt (${neden.toLocaleLowerCase('tr-TR')})`).join(' · ')}
          </div>
        )}

        {testMode === true && (
          <div className="mx-5 mt-3 flex items-start gap-2 rounded-[10px] px-3 py-2.5 text-[12.5px]" style={{ background: 'rgba(251,191,36,0.10)', border: '1px solid rgba(251,191,36,0.35)', color: '#fde68a' }}>
            ...
            <div>
              ...
              <Link href={ILETIM_AYAR_YOLU} className="underline underline-offset-2" style={{ color: '#fcd34d' }}>İletim ayarları</Link>
            </div>
          </div>
        )}

        <div className="mt-5 flex items-center justify-end gap-2 px-5 pb-5">
          <button type="button" onClick={onKapat} disabled={gonderiliyor} className="inline-flex h-9 items-center rounded-[9px] px-3.5 text-[12.5px] font-semibold disabled:opacity-50" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: 'rgba(250,250,249,0.72)' }}>
            Vazgeç
          </button>
          <button
            type="button"
            ...
            style={{ background: 'linear-gradient(135deg, #f4c451, #e0a93c)', color: '#1a1407', boxShadow: '0 10px 22px -12px rgba(244,196,81,0.6)' }}
          >

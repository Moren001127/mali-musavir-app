...
function Metric({ label, value, text, tone = 'neutral', suffix }: { label: string; value?: number; text?: string; tone?: 'neutral' | 'good' | 'bad'; suffix?: string }) {
  ...
  return (
    <div className="rounded-xl p-4" style={portalStyle({ background: tone === 'bad' ? 'rgba(248,113,113,0.06)' : 'rgba(255,255,255,0.02)', border: `1px solid ${tone === 'bad' ? 'rgba(248,113,113,0.22)' : 'rgba(255,255,255,0.07)'}` })}>
      <div className="text-[10.5px] font-bold uppercase tracking-[0.10em]" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>{label}</div>
      <div className="mt-2 text-[24px] font-semibold tabular-nums" style={portalStyle({ color })}>{text ?? fmtTRY(value || 0)}</div>
      {suffix && <div className="mt-1 text-[11px] font-medium" style={portalStyle({ color })}>{suffix}</div>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <PageTitle ust="Ofis" baslik="Cari Hesabım" icon={Wallet} right={
        <button type="button" onClick={ekstreYazdir} className="inline-flex items-center gap-1.5 h-[36px] px-3 rounded-[10px] text-[12.5px] font-semibold transition hover:brightness-110" style={portalStyle({ background: 'rgba(212,184,118,0.14)', border: '1px solid rgba(212,184,118,0.3)', color: '#d4b876' })}>
            ...
            <table className="w-full min-w-[720px] text-sm">
              ...
              <tbody>
                {hareketler.map((h, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t" style={portalStyle({ borderColor: 'rgba(255,255,255,0.055)' })}>
                      <td className="px-4 py-3 text-[12.5px] tabular-nums whitespace-nowrap" style={portalStyle({ color: 'rgba(250,250,249,0.55)' })}>{fmtTarih(h.tarih)}</td>
                      <td className="px-4 py-3">
                        <span className="rounded-md px-2 py-1 text-[10.5px] font-bold" style={portalStyle(tahsilat ? { color: YESIL, background: 'rgba(74,222,128,0.10)' } : { color: '#d4b876', background: 'rgba(212,184,118,0.10)' })}>{TIP_ETIKET[h.tip] || h.tip}</span>
                      </td>
                      <td className="px-4 py-3 text-[12.5px] max-w-[360px] truncate" style={portalStyle({ color: '#fafaf9' })}>{aciklama}</td>
                      <td className="px-4 py-3 text-right text-[12.5px] font-semibold tabular-nums" style={portalStyle({ color: h.borc ? KIRMIZI : 'rgba(250,250,249,0.3)' })}>{h.borc ? `${fmtTRY(h.borc)}` : '—'}</td>
                      <td className="px-4 py-3 text-right text-[12.5px] font-semibold tabular-nums" style={portalStyle({ color: h.alacak ? YESIL : 'rgba(250,250,249,0.3)' })}>{h.alacak ? `${fmtTRY(h.alacak)}` : '—'}</td>
                      <td className="px-4 py-3 text-right text-[12.5px] font-semibold tabular-nums" style={portalStyle({ color: '#fafaf9' })}>{h.runningBakiye != null ? `${fmtTRY(h.runningBakiye)}` : '—'}</td>
                    </tr>
                  );
                })}
              </tbody>

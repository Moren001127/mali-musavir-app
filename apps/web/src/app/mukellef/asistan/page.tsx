...
export default function MukellefAsistan() {
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl overflow-hidden flex flex-col" style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(184,160,111,0.2)', height: 'calc(100vh - 190px)', minHeight: 520 })}>
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {m.role === 'user' ? (
                <div className="max-w-[80%] px-3.5 py-2.5 text-[13.5px] leading-relaxed whitespace-pre-wrap"
                  style={portalStyle({ background: `linear-gradient(135deg, ${GOLD}, #b8a06f)`, color: '#0f0d0b', borderRadius: '14px 14px 4px 14px' })}>
                  {m.text}
                </div>
              ) : (
                <div className="moren-md max-w-[88%] px-3.5 py-2.5 text-[13.5px]"
                  style={portalStyle({ background: 'rgba(255,255,255,0.05)', color: '#fafaf9', borderRadius: '14px 14px 14px 4px', border: '1px solid rgba(255,255,255,0.06)' })}>
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{m.text}</ReactMarkdown>
                </div>
              )}
            </div>
          ))}
          {chatMut.isPending && <div className="flex justify-start"><div className="px-3.5 py-2.5 rounded-2xl" style={portalStyle({ background: 'rgba(255,255,255,0.05)' })}><Loader2 size={15} className="animate-spin" style={portalStyle({ color: GOLD })} /></div></div>}
          {messages.length <= 1 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {sorular.map((s) => (
                <button key={s} type="button" onClick={() => ask(s)}
                  className="text-[12px] px-3 py-1.5 rounded-full" style={portalStyle({ background: 'rgba(184,160,111,0.08)', border: '1px solid rgba(184,160,111,0.2)', color: 'rgba(250,250,249,0.75)' })}>
                  {s}
                </button>
              ))}
            </div>
        ...
        <form onSubmit={send} className="flex items-center gap-2 p-3" style={portalStyle({ borderTop: '1px solid rgba(255,255,255,0.06)' })}>
          <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Sorunuzu yazın…"
            className="flex-1 px-3.5 py-2.5 text-[13.5px] rounded-xl outline-none" style={portalStyle({ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#fafaf9' })} />
          <button type="submit" disabled={!input.trim() || chatMut.isPending} className="flex h-10 w-10 items-center justify-center rounded-xl disabled:opacity-40" style={portalStyle({ background: `linear-gradient(135deg, ${GOLD}, #8b7649)`, color: '#0f0d0b' })}><Send size={16} /></button>
        </form>
      </div>
    </div>
  );

...
export function BelgePreviewHost() {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4" style={portalStyle({ background: 'rgba(0,0,0,0.72)' })} onClick={() => setSt(null)}>
      <div className="flex h-[min(92vh,920px)] w-full max-w-[1100px] flex-col overflow-hidden rounded-2xl" style={portalStyle({ background: '#0f0d0b', border: '1px solid rgba(255,255,255,0.1)' })} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between gap-3 px-4 py-3" style={portalStyle({ borderBottom: '1px solid rgba(255,255,255,0.08)' })}>
          <div className="flex items-center gap-2 min-w-0">
            <Eye size={15} style={portalStyle({ color: GOLD })} />
            <span className="text-[14px] font-semibold truncate" style={portalStyle({ color: '#fafaf9' })}>{st.title || 'Belge'}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {zoomlanabilir && (
              <div className="flex items-center gap-1 mr-1 rounded-lg px-1 py-0.5" style={portalStyle({ border: '1px solid rgba(255,255,255,0.1)' })}>
                <button type="button" onClick={() => setZoom((z) => Math.max(0.4, +(z - 0.25).toFixed(2)))} title="Uzaklaştır" className="flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-white/[0.06]" style={portalStyle({ color: 'rgba(250,250,249,0.8)' })}><ZoomOut size={15} /></button>
                <span className="text-[11.5px] tabular-nums w-9 text-center" style={portalStyle({ color: 'rgba(250,250,249,0.6)' })}>%{Math.round(zoom * 100)}</span>
                <button type="button" onClick={() => setZoom((z) => Math.min(4, +(z + 0.25).toFixed(2)))} title="Yakınlaştır" className="flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-white/[0.06]" style={portalStyle({ color: 'rgba(250,250,249,0.8)' })}><ZoomIn size={15} /></button>
                <button type="button" onClick={() => setZoom(1)} title="Sığdır (genişlik)" className="flex h-7 w-7 items-center justify-center rounded-md transition hover:bg-white/[0.06]" style={portalStyle({ color: 'rgba(250,250,249,0.8)' })}><Maximize2 size={14} /></button>
              </div>
            )}
            {st.url && (
              <a href={st.url} target="_blank" rel="noopener noreferrer" title="Yeni sekmede aç" className="flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-white/[0.06]" style={portalStyle({ border: '1px solid rgba(212,184,118,0.25)', color: GOLD })}><ExternalLink size={15} /></a>
            )}
            <button type="button" onClick={() => setSt(null)} title="Kapat" className="flex h-8 w-8 items-center justify-center rounded-lg transition hover:bg-white/[0.06]" style={portalStyle({ border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(250,250,249,0.7)' })}><X size={16} /></button>
          </div>
        </div>
        <div className="flex-1 min-h-0 overflow-y-auto" style={portalStyle({ background: st.ozet ? '#0f0d0b' : '#1a1a1a' })}>
          {st.loading ? (
            <div className="h-full flex flex-col items-center justify-center gap-3">
              <Loader2 size={26} className="animate-spin" style={portalStyle({ color: GOLD })} />
              {st.title?.includes('özet') ? <span className="text-[12px]" style={portalStyle({ color: 'rgba(250,250,249,0.5)' })}>Belge okunuyor, özet hazırlanıyor…</span> : null}
            </div>
          ) : st.error ? (
            <div className="h-full flex items-center justify-center text-[13px]" style={portalStyle({ color: 'rgba(250,250,249,0.6)' })}>{st.error}</div>
          ) : st.ozet ? (
            <div className="moren-md px-5 py-4 text-[13.5px]" style={portalStyle({ color: '#fafaf9' })}><ReactMarkdown remarkPlugins={[remarkGfm]}>{st.ozet}</ReactMarkdown></div>
          ) : st.url ? (
            <div className="w-full h-full overflow-auto" style={portalStyle({ background: '#0f0d0b' })}>
              {isGorsel
                ? <img src={st.url} alt={st.title || 'Belge'} style={portalStyle({ width: `${zoom * 100}%`, height: 'auto', display: 'block', margin: '0 auto' })} />
                : <iframe src={st.url} title={st.title || 'Belge'} style={portalStyle({ border: 0, background: '#fff', width: `${100 / zoom}%`, height: `${100 / zoom}%`, minHeight: '100%', transform: `scale(${zoom})`, transformOrigin: '0 0' })} />}
            </div>
          ) : null}
        </div>
      </div>
...
export function GozBtn({ onClick, title = 'Görüntüle' }: { onClick: () => void; title?: string }) {
  return (
    <button
      ...
      style={portalStyle({ border: '1px solid rgba(212,184,118,0.28)', color: GOLD, background: 'rgba(212,184,118,0.08)' })}
    >
      <Eye size={15} />
    </button>
  );
    ...
    >
      ...
      {ust ? (
        <div className="mb-3 flex items-center gap-2.5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[.18em]" style={portalStyle({ color: '#b8a06f' })}>{ust}</span>
        </div>
      ) : null}
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3.5">
          {Icon ? (
            <span className="grid shrink-0 place-items-center rounded-xl" style={portalStyle({ width: 46, height: 46, background: `linear-gradient(135deg, ${GOLD}, #b8a06f)`, boxShadow: '0 8px 22px rgba(212,184,118,0.30)' })}>
              <Icon size={24} style={portalStyle({ color: '#1a1410' })} />
            </span>
          ) : null}
          <div className="min-w-0">
            <h1 style={portalStyle({ fontFamily: 'Fraunces, Georgia, serif', fontSize: 28, fontWeight: 600, color: '#fafaf9', letterSpacing: '-.03em', lineHeight: 1.08 })}>{baslik}</h1>
            {aciklama ? (
              <div className="mt-2 flex flex-wrap items-center gap-2">
                {typeof aciklama === 'string'
                  ? <span className="inline-flex items-center rounded-full px-2.5 py-1 text-[11.5px] font-semibold" style={portalStyle({ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(250,250,249,0.72)' })}>{aciklama}</span>
                  : aciklama}
              </div>
            ) : null}
          </div>
...
export function StatStrip({ items }: { items: { label: string; value: string; sub?: string; icon: any; accent?: string }[] }) {
  return (
    <div className="flex items-stretch flex-wrap rounded-[14px] overflow-hidden" style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' })}>
      {items.map(({ label, value, sub, icon: Icon, accent = GOLD }, idx) => (
        ...
        >
          <div className="w-9 h-9 rounded-[10px] flex items-center justify-center flex-shrink-0" style={portalStyle({ background: `${accent}14`, border: `1px solid ${accent}2e`, color: accent })}>
            <Icon size={16} />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] uppercase font-bold tracking-[.1em] truncate" style={portalStyle({ color: 'rgba(250,250,249,0.35)' })}>{label}</p>
            <p className="leading-tight tabular-nums truncate" style={portalStyle({ fontWeight: 700, fontSize: 18, letterSpacing: '-0.02em', color: '#fafaf9' })}>
              {value}
              {sub ? <span className="ml-1.5 text-[11px] font-medium" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>{sub}</span> : null}
            </p>
          </div>
        </div>
      ))}
...
export function Card({ children, accent, className = '', pad = true }: { children: React.ReactNode; accent?: string; className?: string; pad?: boolean }) {
  return (
    <div
      ...
      style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' })}
    >
      {accent ? <div className="absolute inset-x-0 top-0 h-[2px]" style={portalStyle({ background: accent, opacity: 0.5 })} /> : null}
      {children}
    </div>
  );
}
...
export function Badge({ color, children, icon: Icon }: { color: string; children: React.ReactNode; icon?: any }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11.5px] font-medium whitespace-nowrap" style={portalStyle({ background: `${color}1a`, color })}>
...
export function THead({ children }: { children: React.ReactNode }) {
  return (
    <thead style={portalStyle({ background: 'rgba(255,255,255,0.025)' })}>
      <tr className="text-[10.5px] font-semibold uppercase" style={portalStyle({ color: 'rgba(250,250,249,0.45)', letterSpacing: '0.12em' })}>
        {children}
      </tr>
    </thead>
  );
...
export function Section({ baslik, aciklama, sag, children }: { baslik: string; aciklama?: string; sag?: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl overflow-hidden" style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' })}>
      <div className="px-4 py-3 flex items-center justify-between gap-3 flex-wrap" style={portalStyle({ borderBottom: '1px solid rgba(255,255,255,0.06)' })}>
        <div>
          <h2 className="text-[15px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>{baslik}</h2>
          {aciklama ? <p className="text-[12px] mt-0.5" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>{aciklama}</p> : null}
        </div>
...
export function Empty({ children }: { children: React.ReactNode }) {
  return <p className="text-[12.5px] py-3" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>{children}</p>;
}
...
export function OzetCard({ icon: Icon, label, value, accent = GOLD, sub, valueColor }: { icon: any; label: string; value: string; accent?: string; sub?: string; valueColor?: string }) {
  return (
    <div className="rounded-2xl p-5" style={portalStyle({ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' })}>
      <div className="flex items-center justify-between">
        <span className="text-[11.5px] font-medium uppercase tracking-[.05em]" style={portalStyle({ color: 'rgba(250,250,249,0.45)' })}>{label}</span>
        <span className="flex h-8 w-8 items-center justify-center rounded-lg shrink-0" style={portalStyle({ background: `${accent}14`, border: `1px solid ${accent}2e`, color: accent })}>
          <Icon size={15} />
        </span>
      </div>
      <div className="mt-3 leading-none tabular-nums" style={portalStyle({ color: valueColor || '#fafaf9', fontSize: 26, fontWeight: 600, letterSpacing: '-0.025em' })}>{value}</div>
      {sub ? <div className="mt-1.5 text-[12px]" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>{sub}</div> : null}
    </div>
  );
}

...
export default function MukellefProfil() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <div className="divide-y" style={portalStyle({ borderColor: 'rgba(255,255,255,0.06)' })}>
          {rows.map(([k, v]) => (
            <div key={k} className="flex items-center justify-between py-3 gap-4">
              <span className="text-[12.5px] uppercase tracking-wider" style={portalStyle({ color: 'rgba(250,250,249,0.4)' })}>{k}</span>
              <span className="text-[13.5px] text-right" style={portalStyle({ color: '#fafaf9' })}>{v}</span>
            </div>
          ))}
        </div>
        <p className="text-[12px] mt-3" style={portalStyle({ color: 'rgba(250,250,249,0.35)' })}>
      ...
      <Card>
        <div className="flex items-center gap-2 mb-4">
          <KeyRound size={15} style={portalStyle({ color: GOLD })} />
          <h2 className="text-[14px] font-semibold" style={portalStyle({ color: '#fafaf9' })}>Şifre Değiştir</h2>
        </div>
        <form onSubmit={submit} className="space-y-3 max-w-md">
          <input type="password" value={mevcut} onChange={(e) => setMevcut(e.target.value)} placeholder="Mevcut şifreniz" autoComplete="current-password" className="w-full px-3.5 py-2.5 text-[13.5px] rounded-xl outline-none" style={portalStyle(inputStyle)} />
          <input type="password" value={yeni} onChange={(e) => setYeni(e.target.value)} placeholder="Yeni şifre (en az 6 karakter)" autoComplete="new-password" className="w-full px-3.5 py-2.5 text-[13.5px] rounded-xl outline-none" style={portalStyle(inputStyle)} />
          <input type="password" value={yeni2} onChange={(e) => setYeni2(e.target.value)} placeholder="Yeni şifre (tekrar)" autoComplete="new-password" className="w-full px-3.5 py-2.5 text-[13.5px] rounded-xl outline-none" style={portalStyle(inputStyle)} />
          {msg && (
            <div className="text-[12.5px] flex items-center gap-1.5" style={portalStyle({ color: msg.tip === 'ok' ? '#4ade80' : '#f87171' })}>
              {msg.tip === 'ok' ? <Check size={14} /> : null}{msg.text}
            </div>
          )}
          <button type="submit" disabled={sifreMut.isPending || !mevcut || !yeni || !yeni2} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-semibold disabled:opacity-40" style={portalStyle({ background: `linear-gradient(135deg, ${GOLD}, #8b7649)`, color: '#0f0d0b' })}>

...
3) Uyumsuzları değerlendir; demirbaş/tevkifat/mükerrer/iade → işaretle, karar verme; emin değilsen hesap BOŞ — fm_uyumsuzluklar → fm_belge_detay → fm_hesap_plani_ara → get_firma_hafizasi → fm_hesap_ata → fm_isaretle — portal_yaz — senkron — her belge: öneri (kaynak AJAN) ya da işaret — KULLANICI satırına dokunma.
4) "Onayınızı bekleyen" listesi (fm_onayla ekibe kapalı) — create_pending_action — portal_yaz — — — pending id — "KAYDEDİLEMEDİ:".
...
Rapor: FATURA MERKEZİ — <Mükellef> <YYYY-MM> / belge <n> · okundu <> · hesap <> · uyumsuz <> / şüpheli: demirbaş <> tevkifat <> mükerrer <> iade <> / Onayınızı bekleyen: <liste> / Luca iş: <jobId, durum | yapılacaktı> / Kuru testte gerçek yapılan işler / Kime döndü: Muzaffer Bey.

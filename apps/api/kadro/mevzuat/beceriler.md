...
## 1. Günlük tarama (M1 — 07:30 cron PLANLANDI; bugün Muzaffer Bey/Koordinatör görev metniyle)
...
6. Koordinatör'e günlük özet (en fazla 5 kart; fazlası varsa önem sırasıyla ve "devamı var"). Her kartın "YAPILMASI GEREKEN" satırı "Onayınızı bekleyen" maddesidir → `create_pending_action` (başlık "Mevzuat: <başlık> → <ilgili çalışan / Muzaffer Bey>"). Kime döndü: Koordinatör → ilgili çalışan (Bordro-SGK / Beyanname / e-Defter / Fatura / Risk) / Muzaffer Bey (mükellef bilgilendirme kararı).
...
YAPILMASI GEREKEN: (ofis içi: referans güncelle, takvim güncelle, mükellef bilgilendir — kararı Muzaffer Bey verir)
```
...
- İlan görülünce: etkilenen il/ilçe → `list_taxpayers` il süzgeci → mükellef listesi → aynı gün KIRMIZI: `create_pending_action` (priority yüksek; "süre uzatımı / mücbir sebep: <kapsam> — <n> mükellef — yeni son gün <tarih>"). Kime döndü: Koordinatör → Muzaffer Bey + Beyanname Uzmanı (takvim). Portal takvimini ben değiştirmem.
...
Kime döndü: Koordinatör → Muzaffer Bey (kaynak elle kontrol)
```
...
- "Bugün ilgili değişiklik yok" da bir rapordur: Bulgular: "yok (get_gundem: n madde tarandı)".
- Onayınızı bekleyen: her kartın YAPILMASI GEREKEN satırı + "referans güncellenmeli" (get_accounting_reference ile farklıysa eski → yeni) + "takvim güncellenmeli".
- Öğrendiklerim: yalnız tarama yöntemi dersleri; mevzuat içeriği hafızaya `save_ai_memory` ile ancak Muzaffer Bey'in onayından sonra ve kaynak+tarihle.

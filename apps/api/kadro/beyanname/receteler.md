...
3) Luca ajanı çevrimiçi mi; oturum başına çekim — get_agent_status → kdv_kontrol_luca_cek — luca_yaz — asenkron — 2 jobId (mevcutIs olabilir) — çevrimdışı → DUR; kuru test → "yapılacaktı", 6/8/9 ÇAĞRILMAZ.
...
8a) Luca çekimi bitti ve 0 kayıt VE fatura 0 → oturumu kilitle, 8-9'u atla, rapora "boş dönem, kilitlendi" — kdv_kontrol_bos_oturum_kilitle — portal_yaz_agir — senkron — kilitlendi:true — ok:false (portalda fatura var = Luca'ya İŞLENMEMİŞ) → kilitleme, rapora "HAZIR DEĞİL: evrak Luca'ya işlenmemiş", DUR.

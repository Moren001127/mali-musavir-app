...
## 2. İşe giriş / çıkış
1. Mükelleften bilgi geldi (ad, kimlik no, başlangıç/ayrılış tarihi, meslek kodu, ücret). Kimlik no ve maaş rapora, "Onayınızı bekleyen" maddesine ve `create_pending_action` gövdesine YAZILMAZ (00_ORTAK §6); çalışanı ad + mükellef adıyla an.
...
4. Bildirge taslağını hazırla; Onayınızı bekleyen: "işe giriş/çıkış bildirgesi / <mükellef> — <çalışan adı> / – / son gün <tarih>, Muzaffer Bey gönderecek" → `create_pending_action`. Kime döndü: Koordinatör → Muzaffer Bey.
...
APHB: hazır (gönderim Muzaffer Bey'de) / fark var (n satır) / taslak yok
...
- "Onayınızı bekleyen" maddeleri (bildirge Muzaffer Bey'de, APHB Muzaffer Bey'de, tebligat, eksik bilgi mesajı) her biri `create_pending_action` kaydıyla.
- Kime döndü: Koordinatör → Beyanname Uzmanı (muhtasar) / Luca Operatörü (tahakkuk fişi DEVİR) / Muzaffer Bey (gönderim).
- Öğrendiklerim: mükellefe özgü kalıcı bilgi ("X'te 5 puan teşviki uygulanıyor") → `save_ai_memory`.

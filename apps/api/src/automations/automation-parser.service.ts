...
2b. **EVENT seçtiysen \`triggerConfig.eventName\` MUTLAKA aşağıdaki listeden BİRİ olmalı:**
   ...
   - Taxpayer.EvrakDurumuChanged / EvrakIslendiChanged / KontrolEdildiChanged / BeyannameDurumuChanged: \`taxpayerId\`, \`taxpayerUnvan\`, \`taxpayerVkn\`, \`year\`, \`month\`, \`periodLabel\` (İŞLEM ayı YYYY-MM, sıfır-dolgulu), \`beyannamePeriodLabel\` (BEYANNAME dönemi = işlem ayı-1, YYYY-MM), \`field\`, \`oldValue\`, \`newValue\`. KRİTİK: Bildirim/mesaj metinlerinde müvekkile gösterilen DÖNEM = \`beyannamePeriodLabel\` (evraklar beyanname dönemine aittir: işlem ayı Haziran ise evrak Mayıs dönemine aittir). \`year\`/\`month\`/\`periodLabel\` işlem ayıdır, müşteriye dönem olarak YAZMA.

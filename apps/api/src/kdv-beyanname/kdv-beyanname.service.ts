    ...
    const sumSection = (kind: 'gelir' | 'gider'): { toplam: number; adet: number } => {
      ...
      for (let i = target.idx + 1; i < sectionEnd; i++) {
        ...
        if (!tarih) {
          ...
          if (Math.abs(kdv) > 0 || /nakli|toplam|yekun|devir|iade|kalan/.test(firstCells)) {
            probeUndated.push(`{${firstCells.slice(0, 60)}|kdv=${kdv}}`); // [IGGPROBE]
          }
          continue;
        }
      ...
      // [IGGPROBE] geçici teşhis — Erdoğan/Celal İHO≠defter farkı çözülünce KALDIR
      // eslint-disable-next-line no-console
      console.log(
        `[IGGPROBE] ${kind} donem=${donem} kdvCol="${kdvIdx >= 0 ? target.cells[kdvIdx] : 'YOK'}"(idx${kdvIdx}) ` +
        `tarihIdx=${tarihIdx} toplamKdv=${toplamKdv} nakliKdv=${nakliKdv} ` +
        `rowSum=${Math.round(rowSum * 100) / 100} adet=${adet} undated=[${probeUndated.join(', ')}]`,
      );
      const toplam =
        toplamKdv != null

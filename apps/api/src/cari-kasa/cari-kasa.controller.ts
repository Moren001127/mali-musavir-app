...
// Ajan anahtarı çözümü ortak dosyada (common/agent-token.ts) — buradaki kopyalar 2026-09-25'te silindi.
...
export class CariKasaAgentController {
  ...
  /**
   * 2026-09-25 — KOPYA KALDIRILDI (denetim bulgusu 01'in ikinci kopyası).
   *
   * Burada ortak `resolveTenantFromAgentToken` mantığı SATIR SATIR kopyalanmıştı. İki sonucu
   * vardı: (1) bulgu 01 ortak dosyada düzeltilirken bu kopya düzelmedi, (2) eski yol kullanım
   * SAYACI ortak dosyada olduğu için buradan gelen çağrılar HİÇ SAYILMIYORDU — "geçiş bitti mi"
   * ölçümü eksik veri üzerinden yapılıyordu. Artık ortak fonksiyon çağrılıyor ve kaynak
   * etiketiyle sayılıyor.
   */
  private async resolveTenantFromAgentToken(token?: string): Promise<string> {
    return resolveAgentTenant(token, this.prisma as any, { kaynak: 'cari-kasa' });
  }
}

...
const TOOL_GROUPS: Array<{ pattern: RegExp; tools: string[] }> = [
  ...
  { pattern: /(kdv|muhtasar|muhsgk|sgk|stopaj|geçici|gecici|damga|kurumlar|gelir vergisi).*(çıkan|cikan|öde|ode|tutar|borç|borc|ödeyecek|odeyecek|çıkıyor|cikiyor|kim|liste)|(kim|hangi|tüm|tum|herkes|liste).*(kdv|muhtasar|muhsgk|sgk|stopaj|geçici|gecici|damga|kurumlar)/i, tools: ['list_tax_payable', 'get_kdv_summary', 'list_taxpayers_monthly_status'] },
  // Vergi türü geçmeyen ama ödeme/tahakkuk listesi soran haller ("kimlere ödeme çıkıyor",
  // "ödeme tutarlarını yaz", "kim ne kadar ödeyecek") → vergi ödeme listesi aracını sun.
  { pattern: /(kim|hangi|tüm|tum|herkes|liste|kaç|kac).{0,20}(öde|ode|tahakkuk|borç|borc).{0,12}(çık|cik|var|kim|yaz|tutar)|(öde|ode|tahakkuk).{0,12}(çıkan|cikan|çıkıyor|cikiyor)|ödeme tutar|odeme tutar|kim ne kadar öde|kim ne kadar ode/i, tools: ['list_tax_payable', 'list_beyan_kayitlari', 'get_collection_risk_summary'] },
  { pattern: /kdv|beyan|muhtasar|muhsgk|kurumlar|damga|tahakkuk|onay no|hattat|verilebil|verilecek|hazir|hazır|geçici|gecici|stopaj/i, tools: ['list_tax_payable', 'get_kdv_summary', 'list_beyan_kayitlari', 'get_beyanname_config', 'get_beyan_ozet', 'get_beyanname_readiness_summary', 'list_taxpayers_monthly_status', 'get_tax_calendar'] },

  ...
  private async ayristir(
    ...
    const sonuc = await claudeTextViaMax({
      ...
      prompt: [
        ...
        '- "ofisten kendime X çektim", "hesaptan hesaba aktardım" → TRANSFER.',
        ...
        '',
        'KURALLAR:',
        '- Ofis işi (müşavirlik ücreti, mükellef tahsilatı, ofis kirası, personel, SGK, kırtasiye) → defter OFIS.',
        '- Kişisel (market, yakıt, giyim, yemek, ev kirası) → defter SAHSI.',
  ...
  private async planlananKaydet(k: Kimlik, c: any, kategoriler: any[], hesaplar: any[]) {
    ...
    return this.basarili(
      ...
      [
        ...
        `Defter: ${this.defterAdi(kayit.defter)}`,
        c.aciklama ? `Açıklama: ${c.aciklama}` : '',
      ],

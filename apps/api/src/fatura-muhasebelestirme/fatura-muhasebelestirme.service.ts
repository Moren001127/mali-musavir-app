...
<Invoice>
  ...
  async aiReadDocument(tenantId: string, documentId: string) {
    ...
    const prompt = [
      ...
      `muhasebeNeden: Bir mali müşavirin ağzından, MÜKELLEFİN BAKIŞ AÇISINDAN AKICI ve DOĞAL Türkçe 1-2 cümlelik değerlendirme (kalıp/şablon DEĞİL — gerçekten yorumla). YÖN KESİN: ${yonBilgi}. ALIŞ ise: mükellefin ALDIĞI mal/hizmetin işi için ne ifade ettiğini yaz (gider mi? stok/ticari_mal mı? hammadde mi? demirbaş mı?) — SATICI'NIN bakışıyla YAZMA ("satış geliri"/"stok olarak satılmış" gibi ifadeler YASAK, bunlar satıcıya ait). SATIŞ ise: mükellefin sattığı içeriği ve gelir niteliğini yaz. ⚠️ Hesap NUMARASINI (153.01.001 gibi) YAZMA — onu sistem ekler. Örnek (ALIŞ): "Firma gıda deposundan meyve, sebze ve süt ürünleri almış; lokantanın mutfak hammaddesi niteliğindedir." Örnek (SATIŞ): "Müşteriye nakliye hizmeti verilmiş; firmanın ana faaliyet gelirine giren işlemdir." Belge neyse onu açıkla, uydurma.`,
      mukellefBilgi

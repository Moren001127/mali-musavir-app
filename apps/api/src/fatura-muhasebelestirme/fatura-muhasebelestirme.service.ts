...
export class FaturaMuhasebelestirmeService implements OnModuleInit, OnModuleDestroy {
  ...
  async prefetchTurmobInboxDocuments(
    ...
    try {
      ...
      // KANITLANMIS YOL (tek calisan): fetchTurmobPortalInvoices KENDI zengin liste cekisinden IdFatura
      //   alir + gerekirse fallback ile indirir. TURMOB sync-listesi (fetchTurmobPortalRows) IdFatura'yi
      //   GUVENILIR VERMEDIGI icin (LOG KANITI: "idsiz 35/47") dogrudan-indirme tutmadi → bu yola donuldu.
      //   20'serli gruplu → her grup bitince satir yazilir (sayac kademeli); her grup TUM listeyi tekrar
      //   ceker (yavas + TURMOB sinirlamasina tabi) AMA indirir. Hata=satir MISSING (takilmaz).
